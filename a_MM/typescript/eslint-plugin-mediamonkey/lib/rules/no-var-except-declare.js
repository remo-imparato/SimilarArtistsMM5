...
const astUtils = require('../eslint-builtin-utils/ast-utils-rules');
...
module.exports = {
    ...
    create(context) {
        ...
        return {
            /**
             * @param {import('estree').VariableDeclaration & import('eslint').Rule.NodeParentExtension} node
             */
            "VariableDeclaration:exit"(node) {
                if (node.kind === "var") {
                    const tokens = sourceCode.getTokens(node);
                    if (tokens.some(token => token.type === 'Identifier' && token.value === 'declare')) return; // Ignore declare
                    
                    /**
                     * @param {import('estree').VariableDeclaration & import('eslint').Rule.NodeParentExtension} node
                     */
                    const parentIncludesGlobalDeclaration = (node, maxDepth) => {
                        if (maxDepth <= 0) return false;
                        if (!node.parent) return false;
                        if (node.parent.type === 'TSModuleDeclaration' && node.parent.global) return true;
                        return parentIncludesGlobalDeclaration(node.parent, maxDepth - 1);
                    }
                    
                    if (parentIncludesGlobalDeclaration(node, 5)) return; // Ignore declare global scope
                    report(node);
                }
            }
        };
