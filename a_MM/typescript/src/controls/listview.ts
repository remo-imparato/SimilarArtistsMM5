...
declare global {
    var setPix: (val) => string;
    
    interface HTMLElement {
        /** @event */
        event_datasourcechanged: (e: CustomEvent) => any;
        /** @event */
        event_sizechanged: (e: NotifyEvent) => any;
        /** @event */
        event_focuschange: (e: NotifyEvent) => any;
    }
}
...
export default class ListView extends Control {
    private _fastObject2: any;
    private _canScrollHoriz: boolean;
    ...
    itemWidth: number;
    itemBoxProperties: {
        height: number; // pure width without padding
        width: number; // pure height without padding
    ...
    private _predrawTimeout: number | undefined;
    _disablePredraw: boolean;
    drawQueued: boolean;
    focusVisible: boolean;
    private _parentOffsetHeight: number;
    ...
    lastHoveredDiv: HTMLDivElement | undefined;
    lastMouseDownDiv: HTMLElement | undefined;
    _isTreeView: boolean;
    ...
    private _lastHoverUpdate: number;
    private _canvasStartRect: { top: any; left: any; width: any; height: any; } | undefined;
    ...
    private _oldSize: any;
    forceRebindAll: boolean;
    forceRebindSelection: boolean;
    scrollingCaption: HTMLDivElement;
    private _isDrawing: any;
    scrollUpdateNeeded: boolean;
    invalidateNeeded: boolean;
    private _searchBuffer: any;
    supressIncrementalSearchToasts: boolean;
    ignoreShiftFocusChange: boolean;
    checkboxes: boolean;
    private _lastSearchBuffer: any;
    inEdit: AnyDict;
    _groupShiftFocusedID: undefined;
    _lastFocusChangingPromise: undefined;
    _lassoSelectionStart: { x: number; y: number; startingItemIndex: any; itemIndex: any; direction: number; offset: number; lvpos: any; headerHeight: number; } | undefined;
    private _requestFocusIndex: number | undefined;
    showRowCount: number;
    ...
    private _containerOffsetHeight: number;
    invalidateScrollPos: boolean;
    _itemToShow: any;
    _requestedFocAndSelectIdx: any;
    private _touchScroll: any;
    private _gumSize: number;
    ...
    oldDropBefore: HTMLDivElement | null;
    popupIndicator: any;
    private _settingDSPromise: any;
    private _recomputePromise: any;
    private _restoreScrollPos: boolean;
    ...
    private _lastComputeOffset: any;
    private _deceleration: number;
    private _velocity: number;
    private _lastTouchPos: any;
    private _originalTouchPos: any;
    private _originalOffset: any;
    private _gumReleaseStep: undefined;
    private _touchTimer: number;
    viewport: HTMLDivElement;
    fill: HTMLDivElement;
    oldDropAfter: any;
    private _gumplayer: Animation;
    private _lastOffset: number;
    private _headerContextMenu: any;
    private _autoSortString: string | undefined;
    private _toolbarActions: import('../actions').ExecutableAction[];
    protected _lastSorting: any;
    isSortable: any;
    private _headerContextMenuHandler: (e: any) => void;
    savedScrollOffset: { topItem: any; origScroll: any; };
    private _notifiedScrollTop: any;
    unlisteners: undefined;
    dontEmitFocusChange: any;
    dontResetState: any;
    private _requestPopup: any;
    private _requestScrollPosition: any;
    headerItems: HTMLDivElement;
    body: HTMLDivElement;
    _targetScrollOffset: number;
    bindFn: any;
    private _releaseGum: () => void;
    private _handleItemChange: any;
    private _handleFocusChanged: (newIndex: any, oldIndex: any) => void;
    private _handleSortChanged: (_itemObjectToShow: any) => void;
    forbiddenWhenLoadedCancel: any;
    _updatesSuspended: any;
    _interactionTimeout: number;
    itemSizes: any[];
    private _regroupSuspended: any;
    lastMouseDiv: any;
    private _selectPromise: any;
    gridPopupDelay: any;
    ...
    initialize(rootelem, params) {
        ...
        this.itemBoxProperties = {
            height: 0, // pure width without padding
            width: 0, // pure height without padding
            paddingLeft: 0,
            paddingRight: 0
        };
        this.rowDimension = -1;
    ...
    _updateHover_RateLimit(x: number, y: number) {
        if (x==-1 && y==-1) {
            if (this.lastHoveredDiv) {
        ...
        let it: HTMLDivElement | undefined = undefined;
        if (itIdx >= 0)
    ...
    */
    getDiv(itemIndex: number): HTMLDivElement | undefined {
        if (itemIndex >= this.firstCachedItem && itemIndex < this.firstCachedItem + this.divs.length) { // this item in in cache
    ...
    getDivFromCache(firstitem, itemindex: number): HTMLDivElement {
        let offset = itemindex - this.firstCachedItem;
        ...
        if (offset < this.divs.length) { // The div is within our cache
            // No need to do anything
        } else { // The div has to be added
            if (offset >= this.maxCachedDivs) { // Cache has to be shifted
                let newFirstItem: number = Math.min(Math.max(itemindex - Math.round(this.maxCachedDivs / 2), 0), firstitem);
    ...
    hideAllDivs() {
        this.divs.forEach(function (div: HTMLDivElement) {
            this.hideDiv(div);
        }.bind(this));
    }
    ...
    hideGroupSepDiv(div: HTMLDivElement) {
        div.style.top = setPix(-2 * (this.groupHeight || 60) - 60); // Move away to not be visible (but not too far, so that it moves in fast during animations)
    }

    setUpTransition(div: HTMLDivElement, attribute: string, finishCallback?: callback) {
    ...
    cancelTransition(div: HTMLDivElement, attribute) {
    ...
    draw_locked() {
        ...
        if (this.forceRebindAll) {
            ...
            this.divs.forEach(function (div: HTMLElement) {
                div.forceRebind = true;
            });
        } else {
        ...
        */
        ...
        do prepareNextSkip(); while (skipAfterIndex < firstitem);
    ...
    drawnow() {
        ...
        if (this._isDrawing) { // #19350
            if (fullLVDebug) ODS('LV: Skipping drawnow because we are inside a synchronous drawnow call already');
            return;
        }
    ...
    renderCollapseMark(div: HTMLElement, group) {
        if (group.collapsed) {
    ...
    notifyControlFocus() {
        ...
        if (ds && ds.count && isUsingKeyboard()) {
            if (this.focusedIndex < 0 /* #15638 */) {
                this.setFocusedAndSelectedIndex(0).then(() => { // #13065, #17849
    ...
    fileTransferPrepare(element: HTMLElement, e: _UIEvent) {
        if (this.dataSource) {
    ...
    getDropMode(e: NotifyEvent): DropMode {
        if (!dnd.isSameControl(e))
            return 'copy';
        return 'move';
    }

    getDropIndex(e: DragEvent) {
        let pos = 0;
    ...
    resizeDiv(div: CustomElement, w: number, h: number) {
    ...
    */
    ...
    setSmoothScrollOffset(newValue: number, canScrollBeyond?: boolean /*To allow scrolling lower than is the current viewport height*/) {
        if (this.dynamicSize && this.scrollingParent) {
    ...
    scrollParentToBestView(headingHeight) {
        ...
        let scTop: number | undefined = undefined;
    ...
    setScrollOffset(newValue: number, canScrollBeyond?: boolean) {
        if (this.dynamicSize && this.scrollingParent) {
    ...
    getItemFromRelativePosition(x: number, y: number, approxResults?: boolean): number {
        if (!this.dynamicSize)
            if (this.isHorizontal)
    ...
    getItemFromAbsolutePosition(x: number, y: number, approxResults?: boolean) {
    ...
    scrollToView(top: number, bottom: number, aboveShift?: number) {
        if (fullLVDebug)
    ...
    setItemFullyVisible(itemIndex: number, immediately?: boolean) {
    ...
    getItemRowUp(itemIndex) {
        ...
        if (this.isGrouped) {
            ...
            if (group && group2 && (group.index != group2.index)) {
                ...
                next = group2.index + Math.min(group2.itemCount - 1,
                    Math.floor((group2.itemCount - 1) / this.itemsPerRow) * this.itemsPerRow + this.getItemColumn(itemIndex));
            }
        }
    ...
    handle_keydown(e) {
        if (this.disabled || !window.popupWindow.readyToHandleHotKey() /*#21080: chromium sometimes send a keystroke to main window even when already handled in menu window*/)
            return;
    ...
    _handleIncrementalSearch(letter: string, reverseOrder?: boolean, nextOccurence?: boolean) {
    ...
    performIncrementalSearch(searchPhrase: string, reverseOrder?: boolean, nextOccurence?: boolean) {
    ...
    _incrementalSearchMessageSuffix(phrase?: string) {
    ...
    */
    ...
    handleItemLongTouch(div: HTMLDivElement, e: NotifyEvent) {
    ...
    handleItemMouseDown(div: HTMLDivElement, e: NotifyEvent) {
        e.stopPropagation();
    ...
    handleLassoStart(div: HTMLDivElement, e: NotifyEvent) {
        ...
        if (this.multiselect && isLeftButton && !e.shiftKey && !e.ctrlKey && !e.altKey && this.lassoSelectionEnabled && !isSelected) {
            if (this.canUseLasso(e)) {
                ...
                this.updateLassoLayer(this._lassoSelectionStart.x, this._lassoSelectionStart.y,
                    this._lassoSelectionStart.x, this._lassoSelectionStart.y);
            }
        }
        return ret;
    }

    updateLassoLayer(fromX: number, fromY: number, toX: number, toY: number) {
    ...
    handleLassoMove(div, e) {
        ...
        if (this._lassoSelectionStart) {
            ...
            if ((currentMouseInfo.x !== this._lassoSelectionStart.x) || (currentMouseInfo.y !== this._lassoSelectionStart.y)) {
                ...
                if ((posY > lvViewportHeight - this.lassoAutoScrollOffset) && (offset < this.viewportSize - lvViewportHeight)) {
            ...
            this.updateLassoLayer(this._lassoSelectionStart.x - (this.isHorizontal ? offset - this._lassoSelectionStart.offset : 0),
                this._lassoSelectionStart.y - (this.isHorizontal ? 0 : offset - this._lassoSelectionStart.offset),
                currentMouseInfo.x, currentMouseInfo.y);
    ...
    handleItemInsert(itemIndex, obj) {
        ...
        if (itemIndex < this.firstVisibleItem) {
            if (this.isGrid)
                this.invalidateAll();
            else {
        ...
        if ((flags === 'flagchange') && (flagData === 1 /* selected */)) {
            ...
            if (this.popupDiv && this.dataSource && this.dataSource.indexOfPersistentIDAsync) {
                ...
                this.dataSource.indexOfPersistentIDAsync(this.popupDiv.itemID).then(function (idx) {
                    ...
                    if (_this.popupDiv && !_this.selectionMode && idx != _this.popupDiv.itemIndex && shownIndex == _this.popupDiv.itemIndex) {
                        _this.showPopup(idx);
                    }
                });
            }
    ...
    */
    ...
    calcPixsPerItems(itemCount: number) {
    ...
    getItemGroup(itemIndex: number, usePositionIndex?: boolean) {
        if (!this.dataSource)
            return undefined;
    ...
    groupsRecompute(reGroup: boolean, reComputeViewport?: boolean, invalidateItemHeight?: boolean) {
    ...
    _adjustSize() {
        ...
        let w, h: number;
        let itemColDim, itemRowDim: number;
        if (this.isHorizontal) {
            w = rect.height;
            h = rect.width;
    ...
    handleBinding(div: HTMLDivElement, index: number) {
        if (this._dataSource) {
            let _this = this;
    ...
    handleBinding_locked(div: HTMLDivElement, index: number) {
    ...
    addItemToCanvas(div: HTMLDivElement) {
    ...
    renderGroupHeader(div: HTMLDivElement, group, forceRebind) {
        div.innerText = group.id;
    }

    // Called in the end to clean up anything registered by the div (group header)
    cleanUpGroupHeader(div: HTMLDivElement) {
        div.parentListView = undefined;
    }

    // Called just once to initialize the group separator
    setUpGroupSep(div: HTMLDivElement) { }

    // Called often to bind data to a group header
    renderGroupSep(div: HTMLDivElement, group: any) { }

    // Called in the end to clean up anything registered by the div (group header)
    cleanUpGroupSep(div: HTMLDivElement) {
        div.parentListView = undefined;
    }
    ...
    onFocusChanged(newfocusedIndex?: number) {
    ...
    createItemsLayout() {
        ...
        let touchend = (e) => {
            ...
            if ((this._velocity > 10 || this._velocity < -10) && (!this._dynamicSize)) {
    ...
    } private _setGum(arg0: boolean, newPosition: any) {
    ...
    */
    ...
    _prepareSortColumns(value: string) {
    ...
    parentScrollFrame(deferDraw?: boolean) {
    ...
    renderPopup(div: HTMLDivElement, item?: any, scrollToView?: boolean): boolean {
