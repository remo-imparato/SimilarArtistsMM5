...
export default class SearchBar extends Control {
    ...
    searchPhrase: string;
    storedScrollOffset: number;
    private _collection: Collection;
    private _lastIncSearchedControlIndex: number;
    QD: QueryData;
    inSearchView: boolean;
    _supressSearchBoxChange: boolean;
    activeViewData: any;
    private __lastSuccessSearchPhrase: string;
    private _lastIncSearchedControl: HTMLElement;
    activeViewControl: HTMLElement;
    activeControlBeforeSearch: HTMLElement;
    ...
    initialize(elem, params) {
        ...
        this.combo = document.createElement('div') as ElementWith<Dropdown>;
                    ...
                    if (newList && this.searchType == 'activelist' && this.searchPhrase != '') {
                        ...
                        view.promise(newList.whenLoaded()).then(() => {
                            if (oldList && oldList.count == newList.count)
                                return;
                            this.requestTimeout(() => { // to be sure that the new unfiltered instance was assigned earlier   
    ...
    _restoreLocalSearch(viewData) {
        ...
        if (this.searchType == 'scrollmatches') {
            ...
            if (this.inSearchView && viewData.reloading && this.isContextualSearch()) // due to #15005
                this._updateSearch();
        }
    }
    ...
    storeState() {
        return {
            mruList: this.combo.controlClass.dataSource.commaText, // most recent used search phrases
            ...
            /*&& isVisible(ctrl)*/ // filter source also for invisible controls -- they can become invisible just temporarily (when filtered source.count = 0)
                ...
                if (cls.dataSource && this.activeViewData && this.activeViewData.reloading) {
                    ...
                    this.activeViewData.promise(cls.dataSource.whenLoaded()).then(() => {
                        cls.requestTimeout(() => { // to be sure that the new unfiltered instance was assigned earlier
    ...
    doIncrementalSearch(reverseOrder?:boolean, nextOccurence?:boolean) {
    ...
    storeScrollOffset() {
        ...
        while (AC) {
            if (AC.controlClass && (AC.controlClass as ListView).getScrollOffset) {
                this.storedScrollOffset = (AC.controlClass as ListView).getScrollOffset();
                break;
            }
            AC = AC.firstElementChild as HTMLElement;
        }
    }
    ...
    restoreScrollOffset() {
        if (this.storedScrollOffset) {
            let AC = this.activeViewControl;
            while (AC) {
                if (AC.controlClass && (AC.controlClass as ListView).setScrollOffset) {
                    (AC.controlClass as ListView).setScrollOffset(this.storedScrollOffset);
                    break;
                }
                AC = AC.firstElementChild as HTMLElement;
            }
    ...
    comeIn(params) {
        ...
        if (searchType == 'activelist' || searchType == 'scrollmatches') {
            ...
            this.activeControlBeforeSearch = AC as HTMLElement; // @ts-ignore
    ...
    _cancelSearch(dontMoveFocus?:boolean) {
        ...
        if (!dontMoveFocus) {
            ...
            this.requestFrame(() => {
                ...
                if (AC && AC.controlClass && AC.controlClass.lastFocusedIndex && isChildOf( this.activeViewControl, AC) /* #19573 */) {   // @ts-ignore
                    AC.controlClass.setFocusedAndSelectedIndex(AC.controlClass.lastFocusedIndex).then(() => { // @ts-ignore
    ...
    set searchType (value) {
        if (value && this._searchType != value) {
            ...
            (this.searchButton.controlClass as IconButton).icon = 'search';
