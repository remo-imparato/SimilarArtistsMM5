...
class Scroller extends Control {
    ...
    scrolled: boolean;
    lassoParentElementScroller: boolean;
    private _ignoreScroll: boolean;
    smoothScroll: boolean;
    disableLastControlEnlarge: boolean;
    smoothScrollTarget: number;
    smoothScrollOrigin: number;
    smoothScrollTime: number;
    frameQueued: boolean;
    private _initialScrollTop: number;
    private _nearestLV: any;
    private _lastClientHeight: number;
    ...
    notifyChildren(deferred?: boolean) {
        let ctrls = qes(this.container, '[data-control-class]');
        for (let i = 0; i < ctrls.length; i++) {
            let el = ctrls[i];
            if (el.controlClass && (el.controlClass as ListView).parentScrollFrame)
                (el.controlClass as ListView).parentScrollFrame(deferred);
        }
    }
    ...
    focusFirstDown(fromPoint: number, maxLen: number, allowEmpty?: boolean) {
        let ctrls = qes(this.container, '[data-control-class]');
    ...
    focusFirstUp(fromPoint: number, maxLen: number, allowEmpty?: boolean) {
    ...
    handle_keydown(e) {
        ...
        case 'Up':
            {
                let el = document.activeElement as HTMLDivElement;
                if (!el)
                    break;
    ...
    _getLastControl(): CustomElement {
        let lastCtrl;
        let ctrl = this.container.firstElementChild as HTMLElement;
        while (ctrl) {
            if (isVisible(ctrl)) {
                lastCtrl = ctrl;
            }
            ctrl = ctrl.nextElementSibling as HTMLElement;
        }
        return lastCtrl;
    }
    ...
    _forward(e, action, forContextMenu?: boolean) {
    ...
    contextMenuHandler(e: NotifyEvent) {
