...
export default class Slider extends Control {
    ...
    private _tickPlacement: string;
    private _tickInterval: number;
    private _seekBarSize: number;
    private _seekBarThumbSizes: { offsetWidth: number; offsetHeight: number; };
    private _seekBarSizes: { offsetWidth: number; offsetHeight: number; };
    private _lastEventTime: number;
    private _minTimeBetweenTicks: number;
    private _seekBarOuter: HTMLDivElement;
    private _ticksOuter: HTMLDivElement;
    private _sliderContainer: HTMLDivElement;
    _seekBar: HTMLDivElement; // not private, used in Waveform descendant
    _seekBarBefore: HTMLDivElement; // not private, used in Waveform descendant
    _seekBarThumb: HTMLDivElement; // not private, used in Waveform descendant
    private _roundToStep: any;
    private _updateInvert: () => void;
    private _assignClasses: any;
    private _updatePos: procedure;
    mouseWheelHandler: any;
    setTabbable: any;
    private _generateTicks: procedure;
    activateImmediateTooltipHandling: boolean;
    sbOuterPos: number;
    private _wheelStep: number;
    private _speechDecimals: number;
    private _speechName: string;
    private _speechUnit: string;
        ...
        this._generateTicks = function () {
            ...
            if ((this._tickPlacement !== 'none') && _validSize()) {
                if (this._tickInterval) {
                    this._ticks = [];
                    for (let i: number = this.min; i <= this.max; i += this.tickInterval) {
                        this._ticks.push(i);
                    }
                }
                if (this._ticks) {
