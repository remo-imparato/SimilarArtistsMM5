...
export default class SearchEditor extends Control {
    ...
    private _loadingNewValues: boolean;
    checkboxes: boolean;
    private _closeEditTm: number;
    private _onEditRow: (e?:any) => void;
    private _removeRow: (row: any) => void;
    private _onRemoveRow: (e?:any) => void;
    private _onBypassRow: (e?:any) => void;
    private _onSwitchDirection: (e?:any) => void;
    private _onEditSort: (e?:any) => void;
    private _onRemoveSort: (e?:any) => void;
    hideNested: boolean;
    ...
    _valueChanged() {
        return new Promise(function (resolve, reject) {
            ...
            if (ViewType == 'qvtFloat') {
                Value1 = cbValue.controlClass.value;
            } else
            if (ViewType == 'qvtRange') {
            ...
            if (ViewType == 'qvtCheckList') {
                Value1 = cbCheckList.controlClass.value;
            } else
            if (ViewType == 'qvtPlaylistTree') {
                resolve();
    ...
    _AddRule2UI(rule, manual, afterRow) {
        ...
        let VTRules = this.qChild('vtRules');
        let row : AnyDict = document.createElement('div');
        row.rule = rule;
        ...
        if (manual) {
            ...
            animTools.animateAddRow(row as HTMLElement);
        }
    ...
    _cleanSearchRow(row, justUnlisten?:boolean) {
        app.unlisten(row);
    ...
    _createOperatorDiv(rule) {
        let _this = this;
        let div : AnyDict = document.createElement('div');
        div.rule = rule;
    ...
    _getAddConditionMenuArray(row) {
        ...
        function _getFields(include, exclude, addSeparator?:boolean) {
            let retAr = [];
    ...
    _onRemoveBranch(_this) { // @ts-ignore
    ...
    _cleanSortRow(row, justUnlisten?:boolean) {
    ...
    _onAddSort(e) {
        ...
        animTools.animateAddRow(row  as HTMLElement, function () {
