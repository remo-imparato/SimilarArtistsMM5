...
const
    PS_TIMEOUT = 3000, // timeout to auto hide player
...
window.nowPlayingLayouts = [
    ...
    },
    {
        ...
        toolbarActions: function (view) {
            return [{
                icon: 'menu',
                identifier: 'playing_menu', // @ts-ignore
...
export default class NowPlayingView extends Control {
    ...
    switcherDiv: HTMLDivElement;
    btnToWindowed: any;
    private _inFullWindowMode: boolean;
    btnToFullWindow: any;
    btnToFullScreen: any;
    btnShowNPList: any;
    layoutDiv: any;
    private _inFullScreenMode: boolean;
    currentLayout: any;
    ...
    restoreView(noAnimation?:boolean) {
        if (this._inFullWindowMode || this._inFullScreenMode) {
    ...
    updateLayout(newLayout) {
        ...
        if (this._inFullWindowMode || this._inFullScreenMode) {
            ...
            uitools.showPlayer((res === PS_ALWAYS_VISIBLE) || (res === PS_ON_MOUSE_MOVE),
                (res === PS_ON_MOUSE_MOVE) ? PS_TIMEOUT : undefined);
        }
    ...
    storeState() {
        let state : AnyDict = super.storeState();
