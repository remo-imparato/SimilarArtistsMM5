...
export default class NowPlayingTracklist extends ColumnTrackList {
    private _autoScrollToCurrentTrack: boolean;
    disabledModeChanges: boolean;
    player: Player;
    private _lastVisible: boolean;
    lastPlaybackState: string;
    private _fastSD: Track;
    private _waitingForLoad: boolean;
    _lastStatusInfo: any;
    _stateToRestore: any;
    _changeRegistered: boolean;
    canSaveNewOrder: boolean;
    lastRefreshDatasourceTm: number;
    _waitingForMainListLoad: boolean;
    ...
    async refreshDatasource(dosync, doforce) {
        ...
        if(!sl)
            return;
        let changed = doforce || (this.dataSource && (this.dataSource.count !== sl.count)) || (this.lastRefreshDatasourceTm>0) || (this.dataSource && !(this.dataSource.hasSameItems(sl)));
        let ds;
        ...
        if(changed) {
            ...
            ODS('--- taking copy');
            ds = sl.getCopyPreserveFlags() as PlaylistEntries;
            ds.resetSortingInfo();
        }
        else {
            ds = this.dataSource;
        ...
        if(lastSortString && (ourTime === this.lastRefreshDatasourceTm)) {
            if(dosync) {
        ...
        if(!this._changeRegistered) {
            ...
            this.localListen(pl, 'change', (eventType, itemIndex, obj, flags) => {
                if(flags === 'flagchange')
                    return;
                let tm = this.visible ? 250 : 2000;
                this.requestTimeout(async () => { // wait a little bit to not call it too often
                    await this.refreshDatasource(false, false);
                    //ODS('--- call rebind');
                    this.rebind();
                }, tm, 'refreshDatasource');
            });
        }
    }
    onShow() {
    ...
    restoreState(fromObject) {
        if(this._waitingForMainListLoad)
            return;
        ...
        let sl = this.player.getSongList();
        if(sl) {
            this._waitingForMainListLoad = true;
            sl.whenLoaded().then(() => {
                this._waitingForMainListLoad = false;
                if(this.dataSource && (this.dataSource.count === sl.count)) {
                    //ODS('--- restoring ' + JSON.stringify(fromObject));
                    super.restoreState(fromObject);
                    this.scrollToCurrentTrack();
                } else if(fromObject) {
                    //ODS('--- saving ' + JSON.stringify(fromObject));
...
registerClass(NowPlayingTracklist);
