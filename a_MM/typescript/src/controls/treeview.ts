...
export default class TreeView extends ListView {
    ...
    editSupport: boolean;
    readOnly: boolean;
    private _changeExpand: any;
    showDeleteButton: boolean;
    onNodeTooltip: any;
    suppliedStoreStateNode: any;
    private _titlesAssigned: boolean;
    private _autoExpandTimer: number;
    private _lastDropNodeIndex: number;
    private _lastDropNodeResult: any;
    private _dropNode: any;
    private _autoExpandingNode: any;
    private _autoExpandingPos: { X: number; Y: number; };
    /** @event */
    event_expandfinish: (e: CustomEvent<{currentNode: Node}>) => void;
    ...
    handle_keydown(e) {
        ...
        case 'Delete':
            ...
            if (!anyModifier && this.checkboxes) {
                ...
                if (this.focusedNode && this.focusedNode.checked) {
                    this.checkFocused();
                    handled = true;
                }
            }
    ...
    collapseNode(node, forced?: boolean) {
        nodeUtils.collapseNode(node, forced);
    }

    collapseAll(forced?: boolean) {
    ...
    editStart() {
        if (this.focusedNode && this.editSupport && nodeUtils.canEditNodeTitle(this.focusedNode)) {            
            ...
            if (fDiv) {
                this.inEdit = {
                    listview: this,
                    editor: editors.gridViewEditors.textEdit, // @ts-ignore
                    div: fDiv.content,
                    fullSize: false, // #15927 - 1a
    ...
    setDragElementData(element: HTMLElement, e: NotifyEvent) {
    ...
    setNodePath(path, noAutoExpand?: boolean, noFocusEvent?: boolean) {
