...
class VideoCollectionView extends BaseCollectionView {
    watchedView: CustomElementWith<VideoGrid>;
    addedView: CustomElementWith<VideoGrid>;
    collQry: string;
    ...
    _getWatched() {
        ...
        let q =
            ' SELECT * FROM Songs WHERE (LastTimePlayed>0) AND ' + this.collQry +
    ...
    _getAdded() {
        ...
        let q =
            ' SELECT * FROM Songs WHERE ' + this.collQry +
