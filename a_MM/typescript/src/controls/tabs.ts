...
export interface TabElement extends HTMLLIElement { 
    isAddNew?: boolean;
    invalid?: boolean;
    parentTabs: Tabs;
...
interface TabCloseBtnElement extends HTMLDivElement {
    showIt?: boolean;
}
...
export default class Tabs extends Control {
    ...
    oldWidth: number;
    oldHeight: number;
    _lastClickedTab: TabElement;
    _checkParentFocus: boolean;
    ...
    initialize(elem: HTMLDivElement, params: AnyDict) {
        ...
        */        
        ...
        let list = qetag(this.container, 'ul') as HCO<HTMLUListElement>;
        if (list.length > 0) {
            this.header = list[0];
    ...
    */
    ...
    getDropTabIndexFromRelativePosition(offsetX: number, offsetY: number) {
        let res;
        ...
        if (icon) {
            loadIcon(icon, function (data) {
                if (document.body.contains(tab.icon)) { // we have still tab in DOM, continue
                    ...
                    if (icon === 'add') setIconAriaLabel(tab.icon, _('New tab'));
                }
            });
        } else {
    ...
    */    
    get items () {
        return filterCondition(qetag(this.header, 'li') as HCO<TabElement>, function (item) {
            return !item.invalid;
        });
    }
    ...
    */    
    get panels () {
        return this.panelsContainer.children as HCO<HTMLDivElement>;
    }
    ...
    */
    ...
    clearTabs() {
        ...
        forEach(this.items, function (tab, i) {
            removeElement(tab);
            removeElement(_this.panels[i] as HTMLElement);
        });
    }
    ...
    */    
    ...
    /**
    Fired when the selected tab is going to be changed. The event can be canceled which results in selection remaining not changed.

    @example
        // Prevents selection of a tab with index 5.
        app.listen( tabs, 'selecting', function (e) {
            if (e.detail.tabIndex == 5)
                e.preventDefault();
        });

    @event selecting
    @param {integer} tabIndex Index of the tab that's going to be selected.
    @param {integer} oldTabIndex Index of the tab that's currently selected.
    */

    /**
    Fired when the selected tab was changed.

    @event selected
    @param {integer} tabIndex Index of the tab that's selected now.
    @param {integer} oldTabIndex Index of the tab that was selected before the change.
    */

    /**
    Fired when a tab is going to be closed. The event can be canceled which results in tab not closed.

    @event closing
    @param {integer} tabIndex Index of the tab that's going to be closed.
    */

    /**
    Fired when a tab was closed.

    @event closed
    @param {integer} tabIndex Index of the tab that was just closed.
    */
