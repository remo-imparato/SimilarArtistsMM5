...
import { Action, MenuItem } from '../actions';
...
export default class BackgroundImage extends Control {
    ...
    imageShown: HTMLDivElement;
    restoreCalled: boolean;
    currentURL: string;
    lastTrack: Track;
    private __trackChangeFunc: (...params: any[]) => void;
    _lastFetchedCoverType: string;
    animateAction: { title: any; checkable: boolean; checked: boolean; visible: () => boolean; execute: any; };
    private _loaderToken: { cancel: () => void; };
    currentIconAnimation: any;
    currentAnimation: any;
    oldAnimation: any;
    showArtistsAction: Action;
    showAlbumAction: Action;
    showAlbumDarkAction: Action;
    showAlbumOriginalAction: Action;
    menuItems: MenuItem[];
    lastAlbumName: string;
    imageLoading: number;
    imgloader: HTMLImageElement;
    ...
    initialize(parentel, params) {
        ...
        for (let i = 0; i < this.container.children.length; i++) {
            let el = this.container.children[i] as HTMLElement;
    ...
    downloadImages() { // Prepare the list of available images
        ...
        return new Promise<void>(function (resolve, reject) {
            ...
            _this.localPromise(artists.whenLoaded()).then(function () {
                artists.locked(function () {
                    if (artists.count <= 0)
                        return reject();
                    let artist = artists.getValue(0) as Artist;
    ...
    downloadGoogle(counter) {
        ...
        return new Promise<void>(function (resolve, reject) {
            ...
            searchTools.searchArtistImages(_this.currentArtist, function (res) {
                ...
                minSize: 1024, //1600 was too high, often not relevant images, #21504
    ...
    showAlbumCover(covertype) {
        ...
        let cancelToken = uitools.getItemThumb(_this.lastTrack, 500, 500, function (path) {
            ...
            canReturnAlbumArtwork: true, // so it can load artwork from whole album (e.g. saved only to cache), not only for this track
    ...
    updateImage() { // Show one of the available images
