...
export class YearTrackList extends ColumnTrackList {} // #18456
...
export default class YearTracks extends Control {
    ...
    private _ds: any;
    private _listChangeHandler: any;
    ...
    setTracklist(getTracklistFunc, mode) {
        ...
        if ((_this.tracklistMode !== mode) || (!UI.yearTracklist.controlClass.dataSource && !_this.loadingTracklist)) {
            ...
            if (_this.tracklistMode && (_this.tracklistMode !== 'topTracks'))
                _this._ds.sortStrings[_this.tracklistMode] = (UI.yearTracklist.controlClass as YearTrackList).sortString;
            ...
            let _setDS = function () {
                ...
                if ((_this.tracklistMode === mode) && (_this._ds.cache[mode])) { // mode not changed yet
                    if (_this.tracklistMode === 'topTracks') {
                        (UI.yearTracklist.controlClass as YearTrackList).autoSortString = 'rating DESC;playCounter DESC'; // prepare right sort string for new data source
                    } else {
                        (UI.yearTracklist.controlClass as YearTrackList).sortString = _this._ds.sortStrings[mode]; // prepare right sort string for new data source
                    }
        ...
        if (this._searchPhrase != phrase) { // to not reload unfiltered list via searchBar > restoreLocalSearch > comeOut > searchText = ''  .. to just hide the search bar (#17855)
            ...
            if (!this.showingAllTracks) {
                ...
                this.readAllTracks().then((tracks) => {
                    ...
                    (this.UI.yearTracklist.controlClass as YearTrackList).autoSortString = this._ds.sortStrings['allTracks']; // prepare right sort string for new data source
                    this.updateView();
                });
            } else
                this.updateView();
    ...
    storeState() {
        let state : ControlState = super.storeState();
        state.showingAllTracks = this.showingAllTracks;
        return state;
    }

    restoreState(state: ControlState) {
    ...
    restorePersistentState(state) {
        ...
        if (this._ds && state.sortStrings) {
            ...
            if (this.showingAllTracks)
                (this.UI.yearTracklist.controlClass as YearTrackList).sortString = this._ds.sortStrings.allTracks;
        }
    }
    ...
    selectAll() {
        return (this.UI.yearTracklist.controlClass as YearTrackList).selectAll();
    }
    ...
    getCurrenSortString() {
        if (this.UI && this.UI.yearTracklist.controlClass)
            return (this.UI.yearTracklist.controlClass as YearTrackList).sortString;
        else
            return '';
    }
    ...
    get dataSource () {
        if (this.tracklistMode && !this.loadingTracklist && this._ds && this.UI && this.UI.yearTracklist.controlClass) {
            this._ds.sortStrings[this.tracklistMode] = (this.UI.yearTracklist.controlClass as YearTrackList).autoSortString;
        }
        return this._ds;
    }
    set dataSource (value) {
        ...
        if (this._ds && !this._ds.sortStrings)
            this.prepareDefaultSortStrings();
        if (!value) {
            if ((this.UI.yearTracklist.controlClass as YearTrackList).reportStatus)
