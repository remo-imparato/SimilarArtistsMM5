...
export default class ArtWindow extends Control {
    track: Track;
    private _minHeightSet: boolean;
    showProperties: boolean;
    private _artworkAR: number;
    private _fieldListeners: any[];
    usePartialOverflow: boolean;
    lblMode: any;
    modeButtons: any;
    viewButtons: any;
    artwork: any;
    ...
    propsLayer: any;
    propsBgLayer: HTMLElement;
    private _fieldsEditable: boolean;
    unknownAA: any;
    private _lastSelectFirstControlPr: Promise<void>;
    private _lastFirstTrackPr: Promise<void>;
    artworkLayer: any;
    artworkObject: SharedObject;
    private __trackChangeFunc: (...params: any[]) => void;
    private __trackGlobalChangeFunc: (...params: any[]) => void;
    private _stylesCache: any;
    tabBox: any;
    saveArtwork: any;
    private _lastNonvisMode: any;
    private _layoutChangeRequest: boolean;
    private _mode: any;
    private _layoutKey: any;
    showArtworkLayer: any;
    private _lyricsSearchTrack: Track;
    lyricsSearchPromise: any;
    propsPromise: Promise<string>;
    settingDSPending: boolean;
    foundLyrics: any;
    lyricsParagraph: HTMLElement;
    saveLyrics: boolean;
    artworkPromise: any;
    hideTimer: number;
    artworkTextRow: any;
    artworkParagraph: any;
    private _autoAASearchTrack: Track;
    private _lastLayout: any;
    ratingControl: any;
    private _tabbableDivs: any[];

    event_detailsModeChanged: (e: CustomEvent) => void;
    layoutSubmenuItems: () => MenuItem[];
    private _focusedIdx: number;
    private _goAway: boolean;
    private _previousFocusedID: string;
    ...
    _getTrackAsync(link) {
        return new Promise<Track>(function (resolve) {
    ...
    showArtworkDetail(evt?) {
        ...
        if (this.artworkObject.objectType == 'cover')
            pars.coverItem = this.artworkObject;
        else
        if (this.artworkObject.objectType == 'album')
    ...
    _getMenuArray() {
        ...
        function isTrackArtworkDisplayed() { // @ts-ignore
                                ...
                                for (let i = 0; i < coverList.count; i++) {
                                    ...
                                    let item = {
                                        ...
                                        checked: function () {
                                            ...
                                            if (_this.artworkObject.objectType == 'cover')
    ...
    focusFirstEnabled(back?: boolean, col?: boolean) {
    ...
    _fillTabbableDivs(fromFocusIn?:boolean) {
        this._tabbableDivs = [];
        if(this._fieldsEditable) {
    ...
    _makeFieldsEditable(value) {
        forEach(qes(this.propsLayer, '[data-control-class=Editable]') as NodeListOf<ElementWith<Editable>>, function (el) {
            if (el.controlClass) {
    ...
    _anyFieldInEdit() {
        ...
        if(!res && document.activeElement) {
            forEach(qes(this.propsLayer, '[data-control-class=Rating]') as NodeListOf<ElementWith<Rating>>, function (el) {
    ...
    handle_keydown(e) {
        ...
        case 'F2':
            if (focusedElement) {
                if(this._fieldsEditable) {
                    let el = focusedElement as HTMLDivElement;
                    if(el.controlClass && (el.controlClass.constructor.name === 'Rating')) {
                        (el as ElementWith<Rating>).controlClass.focus(true);
                    }
                }
                e.stopPropagation();
            }
                ...
                if(this._fieldsEditable) {
                    ...
                    if(el.controlClass) {
                        if(el.controlClass.constructor.name === 'Rating')
                            ...
                            (el as ElementWith<Editable>).controlClass.startEditMode();
                    }
                }
                handled = false;
            }
    ...
    getFontSize() {
        ...
        if (s.fontSize == 'smallest')
            size = '0.7em';
        else
        if (s.fontSize == 'larger')
            size = '1.2em';
        else
        if (s.fontSize == 'largest')
            size = '2em';
        return size;
    }
        ...
        if (!origTrack.isVideo) {
            if (((origTrack.idalbum > 0) || (origTrack.album && (origTrack.albumArtist || origTrack.artist))) && (this.artworkObject.objectType !== 'album')) {
                ...
                app.getObject('album', {
                    ...
                    canCreateNew: isCD, // #16243 / item 5
    ...
    _loadArtworkLayer(newAssignment?: boolean, fromSearch?: boolean, fromAutoSearch?: boolean, afterError?: boolean) {
        if (this._cleanUpCalled)
            return;
        ...
        if (this.artworkObject && this.showArtworkLayer) {
            ...
            this.hideTimer = this.requestTimeout(() => { //#19262, in case getting artwork lasts too log, hide previous
                setVisibilityFast(this.artwork, false);
                setVisibilityFast(this.unknownAA, true);
    ...
    set dataSource(track) {
        let notSameTracks = ((!this.track && track) || (this.track && !track) || (this.track && track && !this.track.isSame(track))) as boolean;
    ...
    setPropertiesLayer: function (artWindow, track, lyrics, newAssignment, newLayout) {
        ...
        if (newLayout) {
            ...
            if (!artWindow.leftArrow) {                
                ...
                (div.controlClass as IconButton).icon = 'leftArrow';
            ...
            if (!artWindow.rightArrow) {
                ...
                initializeControl(div);
                parentLayer.appendChild(div);
                (div.controlClass as IconButton).icon = 'rightArrow';
                div.controlClass.localListen(div, 'click', (e) => {
                    artWindow.selectCoverIndex(artWindow.currentCoverIndex + 1);
                    e.stopPropagation();
                });
...
window.artDetailsLayouts.advanced = {
    ...
    setPropertiesLayer: function (artWindow, track, lyrics, newAssignment) {
        ...
        if (newAssignment) {
            ...
            let _HTML =
                '<div class="flex row" style="font-size: ' + fontSize + '">';
            if (column1_HTML != '')
                ...
                if (newAssignment) {
                    let lyricsEditFunc = function (e) {
                        ...
                        if (value.trim() === '')
                            value = '';
                        let track = artWindow.track;
                        track.setLyricsAsync(value, (!!artWindow.foundLyrics && !value)).then(function () { // force setting empty lyrics, so no search flag can be set, #18645
                            artWindow.foundLyrics = undefined;
                            track.commitAsync();
                        });
                    };
