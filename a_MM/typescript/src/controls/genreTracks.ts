...
class GenreTrackList extends ColumnTrackList{} // #18456
...
class GenreTracks extends Control {
    ...
    tracklistMode: string;
    loadingTracklist: boolean;
    showingAllOnlineTracks: boolean;
    private _ds: any;
    _listChangeHandler: (...params: any[]) => void;
    allTracksCount: number;
    private _dataSourceChangeHandler: (...params: any[]) => void;
    allOnlineTracksRead: boolean;
    myMusicSet: boolean;
    ...
    initialize(rootelem, params) {
        ...
        (tl.controlClass as GenreTrackList).getDefaultSortString = function () {
    ...
    setTracklist(getTracklistFunc, mode) {
        ...
        if (_this.tracklistMode !== mode) {
            ...
            if (_this.tracklistMode && (_this.tracklistMode != 'topTracks'))
                _this._ds.sortStrings[_this.tracklistMode] = (UI.genreTracklist.controlClass as GenreTrackList).sortString;
            ...
            let _setDS = function () {
                ...
                if ((_this.tracklistMode === mode) && (_this._ds.cache[mode])) { // mode not changed yet
                    if (_this.tracklistMode === 'topTracks') {
                        (UI.genreTracklist.controlClass as GenreTrackList).autoSortString = _this._ds.defaultSortStrings[mode]; // prepare right sort string for new data source
                    } else {
                        (UI.genreTracklist.controlClass as GenreTrackList).sortString = _this._ds.sortStrings[mode]; // prepare right sort string for new data source
                    }
        ...
        if (this._searchPhrase != phrase) { // to not reload unfiltered list via searchBar > restoreLocalSearch > comeOut > searchText = ''  .. to just hide the search bar (#17855)
            ...
            if (!this.showingAllTracks) {
                ...
                this.readAllTracks().then((tracks) => {
                    ...
                    (this.UI.genreTracklist.controlClass as GenreTrackList).autoSortString = this._ds.sortStrings['allTracks']; // prepare right sort string for new data source
                    this.updateView();
                });
            } else
                this.updateView();
    ...
    restorePersistentState(state) {
        ...
        if (this._ds && state.sortStrings) {
            ...
            if (this.showingAllTracks)
                (this.UI.genreTracklist.controlClass as GenreTrackList).sortString = this._ds.sortStrings.allTracks;
        }
    }
    ...
    selectAll() {
        return (this.UI.genreTracklist.controlClass as GenreTrackList).selectAll();
    }
    ...
    getCurrenSortString() {
        if (this.UI && this.UI.genreTracklist.controlClass)
            return (this.UI.genreTracklist.controlClass as GenreTrackList).sortString;
        else
            return '';
    }
    ...
    get dataSource () {
        ...
        if (this.tracklistMode && !this.loadingTracklist && this._ds && this.UI && this.UI.genreTracklist.controlClass) {
            this._ds.sortStrings[this.tracklistMode] = (this.UI.genreTracklist.controlClass as GenreTrackList).autoSortString;
        }
    ...
    set dataSource (obj) {
        ...
        if (obj) {
            ...
            if ((this.UI.genreTracklist.controlClass as GenreTrackList).reportStatus)
