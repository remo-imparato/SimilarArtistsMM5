...
class GenreArtists extends Control {
    ...
    myMusicSet: boolean;
    private _ds: any;
    isSomeRelatedArtist: boolean;
    _dataSourceChangeHandler: (...params: any[]) => void;
    showingOnline: boolean;
    private _relatedArtistsCancelToken: AsyncLoopToken;
    private _artistlistChangeHandler: (...params: any[]) => void;
    onlineartists: any;
    ...
    initialize(rootelem, params) {
        ...
        this.localListen(UI.showTopArtists, 'click', () => {
            _this._updateArtistsTitle(UI.artistlist.controlClass.dataSource.count, (UI.artistlist.controlClass as ArtistGrid).oneRow);
        });
        addEnterAsClick(this, UI.showTopArtists);

        this.localListen(UI.artistSortByContainer, 'change', () => {
            let popupShownList: ArtistGrid = undefined;
            if(_this.showingOnline && !_this.disabledOnline) {
                if((UI.onlineartistlist.controlClass as ArtistGrid).isPopupShown())
                    popupShownList = UI.onlineartistlist.controlClass as ArtistGrid;
            } else {
                if((UI.artistlist.controlClass as ArtistGrid).isPopupShown())
                    popupShownList = UI.artistlist.controlClass as ArtistGrid;
            }
            if(popupShownList) {
        ...
        (UI.artistlist.controlClass as ArtistGrid).oneRow = true;
    ...
    _updateTitle(listCtrl, titleCtrl, cnt, expanded) {
        ...
        if (cnt > (UI.artistlist.controlClass as ArtistGrid).itemsPerRow) {
            titleCtrl.loadedText += ' (' + cnt + ')';
            titleCtrl.innerText = titleCtrl.loadedText;            
            if (expanded)
                (UI.showTopArtists.controlClass as IconButton).icon = 'showLess';
            else
                (UI.showTopArtists.controlClass as IconButton).icon = 'showMore';
    ...
    _updateArtistsTitle(cnt: number, expanded?:boolean) {
        ...
        if (!showOnline) {
            if (!_this.myMusicSet && _this.genre) {
                ...
                if (_this._ds && _this._ds.cache.artistlist)                    
                    _this._updateArtistsTitle(_this._ds.cache.artistlist.count, !(UI.artistlist.controlClass as ArtistGrid).oneRow);
            }
        } else {
            if (this._ds) {
    ...
    set dataSource (obj) {
        ...
        if (obj) {
            ...
            if (!this.showingOnline) {
                if ((this.UI.artistlist.controlClass as ArtistGrid).reportStatus)
                    this.UI.artistlist.controlClass.setStatus('');
            } else if ((this.UI.onlineartistlist.controlClass as ArtistGrid).reportStatus)
