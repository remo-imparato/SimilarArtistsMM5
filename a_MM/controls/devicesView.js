...
class DevicesView extends Control {
    initialize(rootelem, params) {
        ...
        this.localListen(this.UI.btnSubmitServer, 'click', function () {
        ...
        uitools.addToolButton(this.UI.btnAddServer, 'add', function () {
            ...
            _this.UI.cbServerProtocol.controlClass.value = 'http';
            _this.UI.cbServerIP.controlClass.value = 'XXX.XXX.XXX.XXX:YYYYY';
            ds = newStringList();
            ...
            _this.UI.cbServerPath.controlClass.value = 'DeviceDescription.xml';
        ...
        mediaSyncDevices.getBy(params).then(function (devices) {
            if (!_this._showAllDevices) {
                ...
                mediaSyncDevices.getBy(params).then(function (recentDevices) {
                    _this.UI.portableDevices.controlClass.showRowCount = Math.max(recentDevices.count, Math.min(2 /* show at least two*/, devices.count));
                });
            }
            else {
                _this.UI.portableDevices.controlClass.showRowCount = 0; // = show all
            }
