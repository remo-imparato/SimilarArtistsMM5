...
(function () {
    ...
    let __removeBlockDiv = function (forceClose) {
        if (__hoverBlockDiv && (forceClose || (activeWindows === 0))) {
            if (__hoverBlockDivEvent)
                app.unlisten(__hoverBlockDiv, 'mousemove', __hoverBlockDivEvent, true);
            if (__hoverBlockDivDownEvent)
                app.unlisten(__hoverBlockDiv, 'mousedown', __hoverBlockDivDownEvent, true);
    ...
    let initializeWindow = function (w, handler, _thisWindow) {
        ...
        if (!w.eventsInitialized && w._handler) {
            ...
            if (w._handler.onmovedhandler)
                w._movedEvent = app.listen(w._thisWindow, 'moved', w._handler.onmovedhandler);
            if (w._handler.onmousestatehandler)
                w._mouseStateChangedEvent = app.listen(w._thisWindow, 'mouseStateChanged', w._handler.onmousestatehandler, true); // PETR: do not close submenu (#14014)
            if (w._handler.onblurhandler)
                w._activatedEvent = app.listen(w._thisWindow, 'activated', w._handler.onblurhandler);
    ...
    let uninitializeWindow = function (w) {
        if (w.eventsInitialized) {
            ...
            if (w._movedEvent)
                app.unlisten(w._thisWindow, 'moved', w._movedEvent);
            if (w._mouseStateChangedEvent)
                app.unlisten(w._thisWindow, 'mouseStateChanged', w._mouseStateChangedEvent, true); // PETR: do not close submenu (#14014)
            if (w._activatedEvent)
                app.unlisten(w._thisWindow, 'activated', w._activatedEvent);
...
class MenuHandler {
    constructor(params) {
        ...
        this.initMenu = function (screenX, screenY, oppositeX) {
            ...
            let openSubmenu = function (action, div, byMouse) {
            ...
            let _generateItems = function (items, justUpdate) {
                ...
                forEach(menuitems, function (item, idx) {
                    ...
                    if (item.separator) {
                        ...
                        if (item.custom) {
                            ...
                            it.icondiv = document.createElement('div');
                            ...
                            if (!disabled) {
                                ...
                                if (action.buttonActions) { // menu item has own buttons, do not use global hover/click
                                    ...
                                    let mouseclick = function (e) {
                                        ...
                                        if (haveCheckbox) {
                                            it.icondiv.controlClass.checked = !it.icondiv.controlClass.checked;
                                            action.changedstate = it.icondiv.controlClass.checked;
                                        }
                                        else {
                ...
                _this.keyDownHandler = function (e) {
                    let handled = true;
                    let newIdx = -1;
                    let findFirstEnabledDown = function (isCyclic) {
                        let j = 0;
                    ...
                    let findFirstEnabledUp = function (isCyclic) {
                        let j = 0;
                        while ((j < _this.divs.length) && (_this.divs[newIdx].hasAttribute('data-disabled'))) {
                            j++;
                            newIdx--;
                    ...
                    let moveFocus = function (step, isCyclic) {
                        let isUp = (step < 0);
                        if (_this.focusedItem) {
...
function divFromSimpleMenu(divToFill, items, groupTitle) {
