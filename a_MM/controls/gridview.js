...
export default class GridView extends ListView {
    initialize(rootelem, params) {
        ...
        let findColDiv = function (cdiv, offsetX) {
            if (cdiv.nodeName === 'INPUT')
                return; // user clicked on checkbox ... do NOT start editing
    ...
    setShortcutsOnItemIndex(newIdx) {
    ...
    saveButtonHandle(newSortString) {
    ...
    */
    restoreColumns(columns, ignoreNewColumns) {
        let myColumns = [];
        let usedColumnTypes = {};
    ...
    */
    setColumnVisibility(columnType, value) {
    ...
    */
    ...
    setUpHeader(header) {
        ...
        let headerDragoverHandler = function (e) {
            if (this._draggedColumn === undefined)
                return;
            ...
            else if (offsetX > totalPos.width - 50) {
                    ...
                    if (this.sortColumns) {
                        for (let i = 0; i < this.sortColumns.length; i++) {
                            if (this.sortColumns[i].columnType == column.columnType) {
                                ...
                                else if (this.sortColumns[i].direction === 'ASC') {
                                    div.removeAttribute('data-sortAsc');
                                    this.sortColumns[i].direction = 'DESC';
                                    div.setAttribute('data-sortDesc', '1');
                                }
    ...
    visColumnsUpdate(forceRefresh, initialCall) {
    ...
    _getPreviousElementSibling(div) {
        if (div) {
            if (div._groupHeader) {
    ...
    */
    ...
    @return {Column} The resulting column or undefined if none.
    */
    getNextVisibleColumn(column, editable) {
    ...
    @return {Column} The resulting column or undefined if none.
    */
    getPrevVisibleColumn(column, editable) {
        return this.getVisibleColumnOffset(column, -1, editable);
    }
    /**
    ...
    */
    ...
    makeColumnFullyVisible(coldiv) {
        ...
        if (coldiv) {
            ...
            else if (scroll + visible.width < divRight) {
                        ...
                        if (div) {
                            ...
                            if (coldiv) {
                                if (coldiv.check && coldiv.check.controlClass) {
                                    coldiv.check.controlClass.checked = !coldiv.check.controlClass.checked;
                                    handled = true;
                                }
                            }
                        }
