...
export default class YearAlbums extends Control {
    ...
    _updateTitle(listCtrl, titleCtrl, cnt) {
        let UI = this.UI;
        let expanded = !UI.albumlist.controlClass.oneRow;
        if (cnt > UI.albumlist.controlClass.itemsPerRow) {
    ...
    setAlbumsDS(ds) {
        ...
        if (ds) {
            if (this.UI.albumlist.controlClass.autoSortString)
                this.UI.albumlist.controlClass.autoSortString = ''; // force no sorting, we sort DS itself
    ...
    initializeView() {
        ...
        this.localListen(UI.showTopAlbums, 'click', function () {
            UI.albumlist.controlClass.oneRow = !UI.albumlist.controlClass.oneRow;
            _this._updateAlbumsTitle(UI.albumlist.controlClass.dataSource.count);
            if (UI.albumlist.controlClass.oneRow)
        ...
        this.localListen(UI.albumSortByContainer, 'change', function () {
            if (UI.albumlist.controlClass.isPopupShown()) {
                UI.albumlist.controlClass.closePopupFast();
