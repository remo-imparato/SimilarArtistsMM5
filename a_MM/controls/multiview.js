...
export default class Multiview extends Control {
    initialize(rootelem, params) {
        ...
        this.toolBtnParams = {};
        let _this = this;
        Object.defineProperty(this.toolBtnParams, 'parent', {
            get: function () {
        ...
        this.localListen(document.body, 'maintabchange', function (e) {
            ...
            else if (isChildOf(e.detail.newTabPanel, this.container)) {
    ...
    _getFocusedLVActions(assignSelf) {
        let lvActions = [];
        if (this.lastFocusedControl && this.lastFocusedControl.controlClass) {
    ...
    _getViewActions(viewData, assignSelf, onlyViewSelection) {
        let _this = this;
        let vActions;
    ...
    */
    viewAs(viewAsId, subViewChange, viewData, params) {
    ...
    _showView(viewData, viewAsId, params) {
        ...
        if ((params && params.setFocus) || (wasFocused)) {
            ...
            if (this.lastFocusedControl && this.lastFocusedControl.controlClass && this.lastFocusedControl.controlClass.lastFocusedControl) // control (probably multiview) tracks own last focused, use it
                this.lastFocusedControl = this.lastFocusedControl.controlClass.lastFocusedControl;
        }
        let handleFocus = function () {
            if (!this._isSingleSubViewChange() || ((params && params.setFocus))) {
                if (isChildOf(this.container, lastActiveElement) && (!useAnimation && isVisible(lastActiveElement))) {
                    lastActiveElement.focus(); // LS: e.g. to not "steal" focus when using arrows in the "Playlist tree" sub-view
                }
                else {
                    if ((params && params.setFocus) || (wasFocused)) {
    ...
    _initDependencies(usedControls, viewData) {
    ...
    _forEachSubView(viewHandler, callback, includeInvisible) {
    ...
    _anySubViewInScroller(viewHandler) {
        let result = false;
    ...
    _anySubViewNotInScroller(viewHandler) {
        let result = false;
        this._forEachSubView(viewHandler, function (subView, placement) {
            if (!placement.inScroller)
                result = true;
    ...
    _anySubViewHasSplitter(viewHandler) {
        let result = false;
        this._forEachSubView(viewHandler, function (subView, placement) {
    ...
    _anySubViewInSideBar(viewHandler) {
        let result = false;
        this._forEachSubView(viewHandler, function (subView, placement) {
    ...
    _createSubControls(parent, position, inScroller, viewHandler) {
        this._forEachSubView(viewHandler, function (subView, placement) {
    ...
    _processVirtualSubControls(parent, viewHandler) {
    ...
    _createViewControls(viewHandler) {
    ...
    _callViewChange(lastView, animations) {
        let lastViewID;
        if (lastView)
        ...
        let info = {
            income: {
                ...
                panel: this.viewPanel,
                usedControls: this.usedControls,
    ...
    _handler_call_onShow(items, viewData) {
        if (this._subViewToHide)
            return;
        let mainViewControl = this._get_main_view_control(items);
        let _this = this;
        function _process(onlyMain, onlyDataInit) {
    ...
    _handler_call_onHide(items, viewData) {
        if (this._subViewToShow)
            return;
        let mainViewControl = this._get_main_view_control(items);
        let _this = this;
