...
class GenreAlbums extends Control {
    ...
    _getTopAlbums(collQuery) {
        ...
        return this.genre.getAlbumList('SELECT DISTINCT albums.* FROM albums, genressongs, songs ' +
            ...
            ' ORDER BY ' + UI.albumSortByContainer.controlClass.orderByString);
    }
    setAlbumsDS(ds) {
        ...
        if (!this._ds)
            return;
        if (ds) {
            if (this.UI.albumlist.controlClass.autoSortString)
                this.UI.albumlist.controlClass.autoSortString = ''; // force no sorting, we sort DS in SQL query
    ...
    _updateTitle(listCtrl, titleCtrl, cnt, expanded) {
        ...
        if (cnt > this.UI.albumlist.controlClass.itemsPerRow) {
            titleCtrl.loadedText += ' (' + cnt + ')';
            titleCtrl.innerText = titleCtrl.loadedText;
            if (expanded)
                this.UI.showTopAlbums.controlClass.icon = 'showLess';
            else
                this.UI.showTopAlbums.controlClass.icon = 'showMore';
    ...
    _updateAlbumsTitle(cnt, expanded) {
        ...
        if (!showOnline) {
            if (!_this.myMusicSet && _this.genre) {
                ...
                if (_this._ds && _this._ds.cache.allAlbums)
                    _this._updateAlbumsTitle(_this._ds.cache.allAlbums.count, !UI.albumlist.controlClass.oneRow);
            }
        }
        else {
    ...
    initializeView() {
        ...
        this.localListen(UI.showTopAlbums, 'click', function () {
            _this._updateAlbumsTitle(UI.albumlist.controlClass.dataSource.count, UI.albumlist.controlClass.oneRow);
        });
        ...
        this.localListen(UI.onlinealbumsHeader, 'click', function () {
            if (_this.shortOnlineAlbums) {
                ...
                UI.onlinealbumlist.controlClass.oneRow = !UI.onlinealbumlist.controlClass.oneRow;
                if (UI.onlinealbumlist.controlClass.oneRow) {
        ...
        this.localListen(UI.albumSortByContainer, 'change', function () {
            ...
            if (_this.showingOnline && !_this.disabledOnline) {
                if (UI.onlinealbumlist.controlClass.isPopupShown())
                    popupShownList = UI.onlinealbumlist.controlClass;
            }
            else {
                if (UI.albumlist.controlClass.isPopupShown())
                    popupShownList = UI.albumlist.controlClass;
            }
            if (popupShownList) {
        ...
        UI.albumlist.controlClass.oneRow = true;
        UI.onlinealbumlist.controlClass.oneRow = true;
        _this.updateViewRequest();
    ...
    set dataSource(obj) {
        ...
        if (obj) {
            ...
            if (!this.showingOnline) {
                if (this.UI.albumlist.controlClass.reportStatus)
                    this.UI.albumlist.controlClass.setStatus('');
            }
            else if (this.UI.onlinealbumlist.controlClass.reportStatus)
