...
export function getFocusedCC(action, params) {
    params = params || {};
    let control = params.parent || window.lastFocusedControl;
    if (!control && action.multiviewControl) { // action has multiview control assigned, try focused control inside multiview
        ...
        if (control && control.controlClass && control.controlClass.lastFocusedControl) // control (probably inner multiview) tracks own last focused, use it
            control = control.controlClass.lastFocusedControl;
    }
    ...
    if (!control) {
        if (window.currentTabControl && window.currentTabControl.multiviewControl) {
            ...
            if (control && control.controlClass && control.controlClass.lastFocusedControl) // control (probably multiview) tracks own last focused, use it
                control = control.controlClass.lastFocusedControl;
            if (!hasRequiredProps(control, params.reqProps)) {
                control = undefined;
            }
 ...
 * @returns {Tracklist}
 */
export function getSelectedTracklist(LV) {
    ...
    if (!tracks || tracks.count == 0) {
        ...
        if (node) {
            if (node.dataSource && node.dataSource.objectType == 'collection')
                ...
                nodes.add(node);
                tracks = nodeUtils.getNodesTracklist(nodes);
            }
        }
    }
    if (tracks) {
 ...
 */
export function openDialog(dialogName, params, callback) {
    let pars = params || {};
...
export function showDeleteDlg(params) {
    params = params || {};
    params.modal = true;
    params.show = false;
...
export function showOptions(selectPanel) {
...
export function showExtensions(params) {
    let dlgPars = params || {};
    dlgPars.modal = true;
...
export function refreshView(timeout) {
    if (timeout) {
        requestTimeout(() => {
            refreshView();
        }, timeout);
...
export function defaultItemAction(lst) {
...
export function isPinnedAsync(item, pinned) {
    return new Promise(function (resolve) {
...
export function addToolButton(parentId, icon, clickFn, tooltip, parentControl) {
...
let wasWindowMaximized;
let wasBorderAndTitlebarDisabled;
 ...
 */
export function openWeb(url, internal, modal) {
    if (internal) {
 ...
 */
export function openWebSearch(searchEngineURL, prop, item, internal, modal) {
 ...
 */
export function getFindMoreFromSameMenu(params) {
    return new Promise(function (resolve, reject) {
...
export function switchMainMenu(alwaysShow) {
    ...
    if (!menuContainer)
        return;
    let menuButton = qid('mainMenuButton');
    assert(menuButton);
...
export function coverLookup(control) {
    return new Promise(function (resolve, reject) {
...
export function coverRemove(control, onlyFocused = false) {
    control = control || window.lastFocusedControl;
...
export function coverSave(control) {
    control = control || window.lastFocusedControl;
    if (control && control.controlClass && control.controlClass.dataSource) {
        let cvr = control.controlClass.dataSource.focusedItem;
        if (!cvr)
...
export function coverShow(control) {
    control = control || window.lastFocusedControl;
    if (control && control.controlClass && control.controlClass.dataSource) {
        let cvr = control.controlClass.dataSource.focusedItem;
        if (!cvr)
...
export function notLocalMediaListSelected(params) {
...
export function moreTracksSelectedAsync() {
...
export function notMoreTracksSelectedAsync() {
    return new Promise((resolve) => {
...
function notLibraryMediaListSelected(params) {
    if (notMediaListSelected(params))
        return true;
    else {
        return new Promise((resolve) => {
...
export function showPlayer(doShow, autoHideTimeout) {
    const playerCtrl = q('[data-mainPlayer]');
    if (playerCtrl) {
...
export function navigateInNewTab(type, ...args) {
...
export function openMediaProperties(dbDrive) {
    return new Promise(function (resolve, reject) {
...
export function isDockedDialog(dialogName) {
    const thisDialog = window[dialogName];
    if (!thisDialog)
        return false;
    return !!thisDialog._isDockedDialog;
}
export function showGoldMessage(message, closeEvent) {
...
export function addLink(addFunc, linkObj, desc) {
    let _this = this;
    let dlg;
    desc = desc || '';
...
export function showReloadConfirm(customCaption) {
    return new Promise(function (resolve, reject) {
...
export function handlePlayAction(action, lst) {
    ...
    if (!lst && currentListView && currentListView.controlClass && currentListView.controlClass.dataSource && (currentListView.controlClass.dataSource.itemObjectType === 'playlistentry')) {
        ...
        let selTracks = lst || action.getTracklist();
        if (!selTracks) {
            return;
        }
        selTracks.whenLoaded().then(function (selTracks) {
            ...
            switch (action.actionType) {
                ...
                case 'playNowShuffledByAlbum':
                    ...
                    if (track) {
                        track.getTrackTypeAsync().then(async function (tt) {
                            if (window._cleanUpCalled)
                                return;
...
export function copyItem(cut) {
    ...
    else if (ds.objectType == 'sharednodelist') {
...
export function pasteItem(control) {
    let ctrl = control;
    if (!ctrl)
...
export function deleteTracklist(selTracks, permanent, useControl) {
...
/** @hidden */
export let wndTrackInfo;
/** @hidden */
export let wndTrackInfoLoaded = false;
export function createTrackInfo(cbk) {
}
export const visMenuItems = {};
// @ts-ignore
...
export function getScrollingParent(el, prevSP) {
    ...
    if (!retval || !isChildOf(retval, el)) {
        ...
        if (!retval) {
            retval = el.offsetParent; // Our direct parent will work for our purposes.
        }
    }
    return retval;
}
...
*/
export function createBoundFocusHandling(ctrl, containerEl, elementIDs) {
    ...
    if (!wasInitialized) {
        ...
        const focusLocalElement = (index, key) => {
            ...
            while (index < 0)
                index += ctrl._localFocusIDsArray.length;
            ...
            for (let i = 0; i < ctrl._localFocusIDsArray.length; i++) {
                ...
                if (controlIndex < 0)
                    controlIndex += ctrl._localFocusIDsArray.length;
                let el = UI[ctrl._localFocusIDsArray[controlIndex]];
                if (el && isVisible(el)) {
                    el.focus();
