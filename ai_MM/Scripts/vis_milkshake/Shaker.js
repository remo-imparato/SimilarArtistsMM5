...
var Shaker = Class.extend({
    ...
    init: function (cw, ch, initPresetPos) {
        this.settings = {
            meshX: 32,
            meshY: 24,
            fps: 60,
        ...
        for (var presetName in Presets) {
            ...
            this.presets[presetName] = new MilkdropPreset(presetName, Presets[presetName],
                this.settings.meshX, this.settings.meshY);
        }

        this.presetPos = initPresetPos;
...
/*        if (this.count % 400 == 0)
    ...
    selectPreset: function (newPos, hardCut) {
        if ((newPos<0) || (newPos >= this.presetNames.length)) return;
        if (!hardCut)
            this.timeKeeper.StartSmoothing();
        this.presetPos = newPos;
        if (!hardCut)
    ...
    selectNext: function (hardCut) {
        this.selectPreset(this.presetPos+1, hardCut);
    },
    ...
    loadPreset: function () {
        var preset = this.presets[this.presetNames[this.presetPos]];
        return preset;
    },
    ...
    resize: function (newWidth, newHeight) {
        this.settings.windowWidth = newWidth;
        this.settings.windowHeight = newHeight;
        if(this.renderer)
            this.renderer.reset(this.settings.windowWidth, this.settings.windowHeight);
    }
});
