...
@extends Control
*/
const T_ONETIME = _('One time'), T_MANUAL = _('Manual'), T_STARTUP = _('At startup'), T_CONT = _('Continuously'), T_BOTH = T_STARTUP + ' & ' + T_CONT;
const AutoDetectText = _('Auto detection');
...
class MonitoredFoldersView extends ListView {
    initialize(rootelem, params) {
        ...
        this.deleteItems = () => {
            ...
            else if (_this.dataSource && (_this.dataSource.focusedIndex >= 0)) {
    ...
    _setTypeColumnWidth(el, longestString) {
        if (!longestString) {
    ...
    setUpDiv(div) {
        let _this = this;
        let _listen = function (object, event, func, capture) {
            ...
            _listen(check, 'click', function (e) {
                ...
                _this._dataSource.modifyAsync(function () {
                    if (check.controlClass)
                        _this._dataSource.setChecked(div.itemIndex, check.controlClass.checked);
                });
                let handler = scanItemHandlers[getHandlerID(div._folder)];
                if (handler.onChecked && check.controlClass)
                    handler.onChecked(div._folder, check.controlClass.checked);
            });
        ...
        div.appendChild(type);
        // get media type
        let mediatypeCont = document.createElement('div');
