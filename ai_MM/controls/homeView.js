...
class HomeView extends BaseCollectionView {
    ...
    _getTopAlbums(view) {
        ...
        let limit = 15;
        let q = 'SELECT albums.id, avg(playcounter) as pc, avg(Songs.rating) as rat, count(*) as cnt, albpopularity(avg(playcounter), avg(Songs.rating), count(*), COALESCE(albums.LastTimeShown, 0.0)) as popul, albums.album, COALESCE(albums.LastTimeShown, 0.0) AS LastTimeShown ' +
            'FROM songs, albums ' +
    ...
    _getTopArtists(view) {
        let limit = 15;
        let q = 'SELECT artists.id, avg(playcounter) as pc, avg(Songs.rating) as rat, count(*) as cnt, artpopularity(avg(playcounter), avg(Songs.rating), count(*), COALESCE(artists.LastTimeShown, 0.0)) as popul, artists.artist, COALESCE(artists.LastTimeShown, 0.0) AS LastTimeShown ' +
