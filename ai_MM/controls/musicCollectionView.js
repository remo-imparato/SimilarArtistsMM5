...
class MusicCollectionView extends BaseCollectionView {
    ...
    _getTopAlbums(view) {
        let limit = 15;
        let q = 'SELECT albums.id, avg(playcounter) as pc, albums.rating as rat, count(*) as cnt, albpopularity(avg(playcounter), albums.rating, count(*), COALESCE(albums.LastTimeShown, 0.0)) as popul, albums.album, COALESCE(albums.LastTimeShown, 0.0) AS LastTimeShown ' +
            'FROM songs, albums ' +
    ...
    _getTopArtists(view) {
        let limit = 15;
        let q = 'SELECT artists.id, avg(playcounter) as pc, avg(rating) as rat, count(*) as cnt, artpopularity(avg(playcounter), avg(rating), count(*), COALESCE(artists.LastTimeShown, 0.0)) as popul, artists.artist, COALESCE(artists.LastTimeShown, 0.0) AS LastTimeShown ' +
