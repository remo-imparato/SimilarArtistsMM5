...
window.settings = {
    ...
    init: function () {
        let ObservableObject = window['ObservableObject'];
...
(function () {
    ...
    if (window.webApp) {
        window._alert = function (msg, scriptName) {
            console.log(msg, scriptName);
        };
        window.alert = function (msg, scriptName) {
    ...
    window.getMessageWithStack = function (exception) {
...
*/
function asyncLoop(fn, valFrom, token, finishedCallback) {
    let i = valFrom || 0;
...
function createNewCustomEvent(eventName, params) {
    let event;
...
function updateLoadedFile(fname, content) {
    return content;
}
/**
 ...
 * @returns
 */
function loadFile(fname, callback, params) {
...
(function () {
    ...
    function requirejs(files, callback, isolate, local, reload) {
        let fileArr;
...
(function () {
    ...
    window._applyLayoutQueryCallbacks = function () {
        ...
        for (let cb of _callbacksQueue.layoutQueryCallbacks) {
            ...
            cb();
            if (i > 0)
                performance.measure(`callback_${i}`, `callback_${i - 1}`, `callback_${i}`);
            i++;
        }
...
function requirejsDeferred(fname) {
...
(function () {
    ...
    window.loadIconFast = function (iconName, callback) {
    ...
    window.loadIcon = function (iconName, callback) {
        if (iconName === undefined || iconName === '')
            return;
...
*/
function getAllUIElements(rootElement) {
    let ret = {};
...
function filterTag(nodeList, tag) {
    let res = [];
    tag = tag.toUpperCase();
 ...
 */
function filterCondition(nodeList, callback) {
    let res = [];
    for (let i = 0; i < nodeList.length; i++) {
 ...
 */
function fastForEach(list, callback) {
 ...
 */
function listGetCommaIDs(list, maxCount) {
    let res = '[';
 ...
 */
function resolveToValue(property, whenUndefined, params, bindObj) {
...
*/
function cleanElement(e, onlyChildren) {
    if (!e)
        return;
    if (!onlyChildren)
        cleanControlClass(e);
...
*/
function removeElement(e, hideParentWhenNoChild) {
    if (!e)
        return;
    ...
    if (e.unlisteners) {
        forEach(e.unlisteners, function (unlistenFunc) {
            unlistenFunc();
        });
        e.unlisteners = undefined;
    }
    app.unlisten(e);
...
function fixScreenCoords(coords, ctrl) {
    let ret = coords;
...
function getScreenCoordsFromEvent(e) {
    if (window.maximized) {
        let add = 0;
...
let _setCoSizeTm;
function setComputedSize(force, secondLoop) {
...
(function () {
    ...
    */
    window.lockedLayout = function (element, callback) {
        enterLayoutLock(element);
        callback();
        leaveLayoutLock(element);
    };
    window.enterLayoutLock = function (element) {
        if (!element)
            element = document.documentElement;
        element.layoutLocked = (element.layoutLocked || 0) + 1;
    };
    window.leaveLayoutLock = function (element) {
        if (!element)
            element = document.documentElement;
    ...
    let scheduledLayoutDownTimeout;
    let scheduledLayoutDownCallbacks = [];
    ...
    window.idleNotifyLayoutChangeDown = function (element) {
        let uniqueLayoutChangeID = Date.now();
        if (!element._scheduledLayoutDown) {
            element._scheduledLayoutDown = true;
    ...
    window.notifyLayoutChangeDown = function (element, uniqueLayoutChangeID) {
        if (!uniqueLayoutChangeID)
...
whenReady(function () {
    ...
    let Events = ['click', 'dblclick', 'mousedown', 'mousemove', 'mouseover', 'mouseout', 'mouseup', 'keydown', 'keypress', 'keyup'];
...
*/
function isVisible(element, includeParents) {
    let vis;
    let disp;
    if (!element)
        return false;
    ...
    if (!disp || !vis)
        return false;
    else if (includeParents || includeParents === undefined) {
...
*/
function setVisibility(element, visible, params) {
    ...
    // notifyLayoutChange() is supposed to be called directly after completion of the animation
        ...
        if ((!params || (params.layoutchange === undefined) || params.layoutchange)
...
*/
function setVisibilityFast(element, visible, params) {
    params = params || {};
...
@return {HTMLElement} Source element.
*/
function toggleVisibility(el, params) {
...
function finishInitializeWindow() {
    ...
    if (window.inspectElementSupported && !window.qUnit && !window.isYoutubeWindow) { // not used also in youtube window, it was causing double context menus there sometimes
        ...
        let Control = window['Control']; // JL: Workaround since we wish not to make mminit.js a module
        if (!bControl) {
    ...
    if (!window.qUnit) {
        ...
        let TooltipController = window['TooltipController']; // JL: Same workaround as above
...
function getFullWidth(control, options) {
    let cs = getComputedStyle(control, null);
...
function getOuterHeight(control, cs, options) {
    if (cs === undefined)
        cs = getComputedStyle(control, null);
...
TRANSLATION_CLASS_ID_BEGIN = '{', TRANSLATION_CLASS_ID_END = '}';
...
(function () {
    ...
    let _storeTranslatedText = function (el, text) {
        if (text.indexOf(TRANSLATION_CLASS_ID_BEGIN) < 0) { // no classes
            ...
            let pos = 0;
            let lastCopy = 0;
            let copyNodes = function (fromPos, toPos) {
...
(function () {
    ...
    window.friendlyKeyName = function (e) {
    ...
    let touchFinishTimeout = undefined;
    let holdIndicatorStartTimeout = undefined;
    let holdIndicator = undefined;
...
*/
function isPromiseFinished(promise) {
...
*/
function isPromiseCanceled(promise) {
...
function loadDevToolsURL(callback) {
    if (!webApp && !isStub) {
...
*/
function emToPx(emNum, parentEl) {
...
@return {Number}
*/
function pxToEm(pxNum, parentEl) {
...
Object.defineProperty(Object.prototype, 'override', {
    ...
    value: function (source) {
        ...
        let getNewMethod = function (property) {
            let newmethod = source[property], oldmethod = this[property];
            let method = function () {
                let fn;
                if (oldmethod && typeof oldmethod !== 'function') {
                    ...
                    let a = [fn], arrLength = a.length, length = arguments.length;
                    while (length--)
                        a[arrLength + length] = arguments[length];
...
function copyObject(obj) {
    ...
    if ((null == obj) || ('object' != typeof obj) || (obj && obj.native))
        return obj;
    // Handle Date
...
function setFocusState(element, state, setFocus) {
...
(function () {
    ...
    let findNearestParentControlType = function (ctrl, lookForScroller) {
        while (ctrl) {
...
function getAbortError(stack) {
    return new AbortError(stack);
}
function isError(x) {
    return x instanceof Error;
}
function isAbortError(x) {
...
function inArray(what, ar, ignoreCase) {
...
function getSortedAsArray(items) {
    let retitems = [];
    let item;
