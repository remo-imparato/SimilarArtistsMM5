...
let __utils = {
    ...
    isMediaServerTrack(track) {
        return track.path.startsWith('uuid://');
    },
    isAudioCDTrack(track) {
        let ext = getFileExt(track.path);
        return (ext == 'cda' || ext == 'CDA');
    },
    isLibraryTrack(track) {
        return (track.id > 0);
    },
    getFirstTrackAsync(tracklist) {
    ...
    getFileSourceIcons: function (track) {
        ...
        */
        ...
        else if (track.cacheStatus == _utils.CACHE_DOWNLOADED)
            icons = ['download', 'cloud']; // downloaded from cloud
        else if (track.cacheStatus == _utils.CACHE_STREAMED) {
        ...
        else if (_utils.isMediaServerTrack(track)) {
            icons = checkLibraryStatus(['server']);
        }
        else if (_utils.isOnlineTrack(track)) {
            icons = checkLibraryStatus(['youtube']);
        }
        else if (_utils.isRemoteTrack(track))
            icons = checkLibraryStatus(['server']);
        else if (_utils.isAudioCDTrack(track))
    ...
    cleanAndValidateURL: function (inputStr) {
        if (!inputStr)
            return '';
...
function convertSpecialToEntities(str) {
...
function replaceAll(old_pattern, new_pattern, str) {
...
function getSimplifiedExtendedTags(extendedTagsString, sep) {
...
function sanitizeHtml(inputStr, extraSelector) {
    if (!inputStr)
        return '';
    ...
    if (inputStr == '<br>')
        return '';
    if (inputStr.indexOf('<body') == -1)
        inputStr = '<body>' + inputStr + '</body>'; //add "body" otherwise some tags are skipped, like <style>
