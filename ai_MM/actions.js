...
let miniPlayer, microPlayer, limitAlbumTrackRows;
/**
...
export const actions = {
    ...
    mood: {
        ...
        assignToSelectedTracks: function (callback) {
            ...
            if (!tracks)
                return;
            localPromise(tracks.whenLoaded()).then((tracks) => {
    ...
    exitFullscreen: {
        ...
        execute: function () {
            if (window.globalVideoSwitcher && window.globalVideoSwitcher.controlClass) {
                let swC = window.globalVideoSwitcher.controlClass;
                            ...
                            ds.locked(function () {
                                if (ds.getValue) {
                                    ...
                                    if (item) {
                                        ...
                                        else if ((objType === 'track') || (objType === 'album'))
                                            added = (item.id > 0);
    ...
    addMediaServer: {
        ...
        _execute: function (url) {
            ...
            app.sharing.addRemoteServerAsync(url).then(function () {
        ...
        execute: function () {
            ...
            messageDlg(sprintf(_('This will auto-configure rules on your router to allow the %s UPnP/DLNA server to be accessible from a public/external network.'), escapeXml(server.name)) + ' ' + _('Configuration is via UPnP/NAT-PMP which only works if this device is on the same network as the router.') + '<br><br>' + _('Do you want to proceed?'), 'Information', ['btnYes', 'btnNo'], {
                ...
                if (result.btnID === 'btnYes') {
                    ...
                    server.makePublicAsync().then(function (publicURL) {
                        ...
                        if (err == 'GET_PUBLIC_IP_ERR')
                            ...
                            msg = msg + _('To resolve, enable UPnP/NAT-PMP in your router configuration or manually configure port forwarding on your router and try again') + ':<br>' + sprintf(_('port %d to IP address %s'), server.port, server.ip);
                        //msg = msg + _('Error') + ': ' + err + '<br><br>';
                        uitools.showConnectError(msg);
                        progress.terminate();
                    });
    ...
    autoTag: {
        ...
        execute: function () {
            let selTracks = this.getTracklist();
            if (selTracks) {
                window.localPromise(selTracks.whenLoaded()).then((selTracks) => {
                    if (selTracks.count)
        ...
        execute: function () {
            ...
            if (cc) {
                ...
                if (track && cc.dataSource) {
                    ...
                    else if (cc.dataSource.objectType == 'tracklist') {
                ...
                mediaSyncDevices.getSyncable().then(function (devices) {
                    ...
                    _utils.getFirstTrackAsync(selTracks).then((track) => {
                                            ...
                                            execute: function () {
                                                let _addItem = (obj) => {
                                                    ...
                                                    if (obj.objectType == 'tracklist')
                                                        list = obj;
                                                    else if (obj.objectType == 'track' || obj.objectType == 'playlistentry') {
                        ...
                        if (actions.exportToFile) { // if not uninstalled
                            result.push({
                                action: {
                                    title: _('Report') + ' (' + _('File list') + '...)',
                                    hotkeyAble: actions.exportToFile.hotkeyAble,
...
export class JSHotkeys {
    ...
    _addHotkey(hotkey, action, global) {
        if (!global)
            global = false;
...
let _limitAlbumTrackRows = false, _limitAlbumTrackRowsLoaded = false;
let osxMenu;
...
if (isMainWindow) {
    ...
    window.getMenuItemByName = function (name) {
        if (name == 'fileMenuItem')
            return fileMenuItem;
        if (name == 'editMenuItem')
            return editMenuItem;
        if (name == 'viewMenuItem')
            return viewMenuItem;
        if (name == 'playMenuItem')
            return playMenuItem;
        if (name == 'toolsMenuItem')
            return toolsMenuItem;
        if (name == 'helpMenuItem')
            return helpMenuItem;
        return -1;
    };
    // @ts-ignore
...
function generateChoosePlayerSubmenu(params) {
    return new Promise(function (resolve, reject) {
        let retval = [];
        ...
        let _generateItems = () => {
            ...
            lst.forEach(function (player) {
                ...
                else if (player.uuid != '' /* not internal player*/) {
...
function handleFolderAction(params) {
...
function generateFolderSubmenu(params) {
    return new Promise(function (resolve, reject) {
...
function generatePlaylistSubmenu(params) {
    return new Promise(function (resolve, reject) {
        let parAction = params.parentMenuAction;
        let retval = [];
        ...
        let playlist;
        let handlePlaylistAction = function () {
            addToSendToMRUList({
                title: _('Send to') + ' ' + this.fullPath,
                mainAction: this.mainAction,
                icon: 'playlist',
        ...
        let handleNewPlaylist = async function () {
...
function handleSendToMail(params) {
    let selTracks;
    if (params.rootMenuAction && params.rootMenuAction.getTracklist)
        selTracks = params.rootMenuAction.getTracklist();
    else
...
function handleSendToM3U(params) {
    let selTracks;
...
function isFocusedPinnedAsync(requiredRes) {
