...
window.mediaSyncHandlers['cloud'] = { // used for cloud services sync (DropBox, OneDrive, Google Drive, Google Play Music)
    icon(device) {
    ...
    getArtworkUrl(device, track) {
        return new Promise((resolve, reject) => {
            ...
            if (sourceInfo.path && (service.folder_based != false) && service.isAuthorized()) {
    ...
    scanContent(device, fillList, purpose) {
        ...
        return new Promise((resolve, reject) => {
            if (service.isAuthorized()) {
                ...
                service.listContent('' /* root */, list, true /* recursive == list all contents in subfolders */).then(
                    (content) => {
                        if (list.count == 0)
    ...
    listPlaylists(device, parent_playlist, params) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (listFunct) {
                listFunct(parent_playlist, params).then(
                    function (content) {
                        for (let i = 0; i < content.playlists.count; i++) {
                            let playlist = content.playlists.getValue(i);
                            playlist.persistentInfo = { // to be stored in navigation history
                                id: playlist.id
                            };
            ...
            if (!ms.bypass) {
                ...
                list.whenLoaded().then(function () {
                    fastForEach(list, function (track) {
                        cloudTools.createSourceInfo(track, service, { // needed for playback to work
    ...
    _scan_intervals() {
        let HOUR = 60 * 60 * 1000;
        return [100 * 365 * 24 * HOUR, // one time
            1, // startup    
            HOUR,
            12 * HOUR,
            24 * HOUR,
        ...
        summary: {
            ...
            save(device, config) {
                ...
                mediaSyncDevices.changeLibraryScanSettings(device, {
                    runAutoSyncAfterScan: true, // for cloud storages we want to run auto sync after scan (#14799)
        ...
        return new Promise(function (resolve, reject) {
            cloudTools.authorize(service).then(function () {
                ...
                mediaSyncDevices.initRegistered().then1(function () {
                    app.devices.getDeviceAsync(profile_id, service.title).then(
                        function (device) {
                            resolve(device);
                        }, reject);
                });
...
window.mediaSyncHandlers['usb'] = { // used for USB connected device sync (iPods, iPhones, Android devices, SD cards, USB memory sticks)
    icon(device: Device) {
        /*if (device.scanning)
    ...
    addCustomNodes(node: SharedNode) {
        let device = node.dataSource;
        if (device.connected) {
    ...
    metadataStorageEnabled(device: Device, metadataStorage: MetadataStorage) {
    ...
    getFolderContent(device: Device, path: string) {
        let pathDelim = this.PATH_SEPARATOR;
        return new Promise(function (resolve, reject) {

            let pathParts = removeFirstSlash(path).split(pathDelim);
            let processPathPart = function (index: number, folder?:SharedFolder) {

                let list : SharedList<SharedFolder>;
                if (index == 0)
    ...
    getMetadataDirectory(metadataStorage: MetadataStorage) {
        return this.PATH_SEPARATOR + 'MediaMonkey' + this.PATH_SEPARATOR + 'meta_files' + this.PATH_SEPARATOR + 'v' + metadataStorage.getVersion();
    },
    startSyncing(sync: MediaSync, mode) {
        return sync.readMetaFiles(sync.device, sync);
    },
    finishSyncing(sync: MediaSync, mode) {
        return sync.exportMetaFiles(sync);
    },
    showContent(device: Device) {
        return device.connected;
    },
    syncEnabled(device: Device) {
        return device.connected && device.enabled;
    },
    _getTrackSyncId(targetPath: string, track: Track) {
    ...
    removeFiles(sync: MediaSync, filePaths: StringList, tracks: Tracklist) {
        let ms = mediaSyncDevices.getMetadataStorage(sync.device);
        fastForEach(tracks, function (track, idx) {
            let path = getValueAtIndex(filePaths, idx);
    ...
    uploadPlaylist(sync: MediaSync, playlist) {
    ...
    tabVisible(page_id, device: Device) {
        if (!window.uitools.getCanEdit())
            return inArray(page_id, ['summary']);
        else {
    ...
    configPage: {
        summary: {
            load: function (device: Device, config) {
...
window.mediaSyncHandlers['server'] = { // used for "writeable" UPnP/DLNA server sync (e.g. MediaMonkey server installed on NAS or Raspberry Pi)
    icon(device) {
        return 'server';
    },
    syncEnabled(device: Device) {
    ...
    initProfileDefaults(device: Device) {
        ...
        convertConfig.rules.setAllChecked(false);
        convertConfig.setSupportedFormatsList(app.utils.createSharedList() as unknown as SharedList<SuppType>); // Unknown (all formats)
        device.autoConvertConfig = convertConfig;
    ...
    _showMigrationWizard(device: Device) {
        return new Promise(function (resolve, reject) {
    ...
    _showContentSelectionDlg(device: Device) {
        return new Promise(function (resolve, reject) {
    ...
    checkProfileUpdate(device: Device) {
    ...
    modifyTrack(track: Track) {
        ...
        if (this.isModifiableTrack(track)) {
            track.getLyricsAsync().then(() => { // just to be sure that long texts (lyrics/comment) has been read (to not assert in _updateTrackMetas when using track.getCommentSync)
    ...
    _getResources(device: Device, track: Track) {
        return new Promise((resolve, reject) => {
            this._getServer(device).then((res) => {
                let sourceInfo = cloudTools.getSourceInfo(track);
                cloudTools.request({
    ...
    getArtworkUrl(device: Device, track: Track) {
        return new Promise((resolve, reject) => {
    ...
    getStreamUrl(device: Device, track: Track) {
    ...
    deletePlaylist(device: Device, playlist) {
        return new Promise<void>((resolve, reject) => {
            this._getServer(device).then((res) => {
                cloudTools.request({
                    method: 'DELETE',
    ...
    removeFile(sync: MediaSync, filePath: string, track: Track) {
    ...
    _createTrackMetas(track: Track) {
        let metas = {
            ...
            year: track.date, // accepts both 2003 and 20030501
            ...
            originalDate: track.origDate, // accepts both 2003 and 20030501
    ...
    _createSourceInfo(track, server, device, metas) {
        track.dontNotify = true;
        let info = {
            sourceType: 'server', // needed for playback purposes
    ...
    _fillTrackFromMetas(track, metas, purpose) {
        track.dontNotify = true;
        if (metas.sync_id) track.sync_id = metas.sync_id;
        if (metas.size) track.fileLength = metas.size;
        if (purpose != 'sync') {
            if (metas.title) track.title = metas.title;
            if (metas.duration) track.songLength = Math.round(metas.duration * 1000);
            if (metas.album) track.album = metas.album;
            if (metas.albumArtists) track.albumArtist = metas.albumArtists.join(';');
            if (metas.artists) track.artist = metas.artists.join(';');
            if (metas.producers) track.producer = metas.producers.join(';');
            if (metas.actors) track.actors = metas.actors.join(';');
            if (metas.authors) track.author = metas.authors.join(';');
            if (metas.directors) track.director = metas.directors.join(';');
            if (metas.publishers) track.publisher = metas.publishers.join(';');
            if (metas.lyricists) track.lyricist = metas.lyricists.join(';');
            if (metas.conductors) track.conductor = metas.conductors.join(';');
            if (metas.genres) track.genre = metas.genres.join(';');
            if (metas.year) track.year = metas.year;
            if (metas.originalTrackNumber) track.trackNumberInt = metas.originalTrackNumber;
            if (metas.originalDiscNumber) track.discNumberInt = metas.originalDiscNumber;
            if (metas.comment) track.setCommentAsync(metas.comment);
            if (metas.playcount) track.playCounter = metas.playcount;
            if (metas.skipcount) track.skipCount = metas.skipcount;
            if (metas.bookmark) track.playbackPos = metas.bookmark;
            if (metas.bpm) track.bpm = metas.bpm;
            if (metas.rating) track.rating = metas.rating;
            if (metas.volumeLevelTrack) track.normalizeTrack = metas.volumeLevelTrack;
            if (metas.volumeLevelAlbum) track.normalizeAlbum = metas.volumeLevelAlbum;
            if (metas.last_time_played) track.lastTimePlayed_UTC = app.utils.timestamp2DateTime(metas.last_time_played);
            if (metas.parental_rating) track.parentalRating = metas.parental_rating;
            if (metas.grouping) track.groupDesc = metas.grouping;
            if (metas.contributors) track.involvedPeople = metas.contributors.join(';');
            if (metas.tempo) track.tempo = metas.tempo;
            if (metas.mood) track.mood = metas.mood;
            if (metas.occasion) track.occasion = metas.occasion;
            if (metas.quality) track.quality = metas.quality;
            if (metas.isrc) track.isrc = metas.isrc;
            if (metas.initialKey) track.initialKey = metas.initialKey;
            if (metas.originalTitle) track.origTitle = metas.originalTitle;
            if (metas.originalArtist) track.origArtist = metas.originalArtist;
            if (metas.originalLyricist) track.origLyricist = metas.originalLyricist;
            if (metas.originalDate) track.origDate = metas.originalDate;
            if (metas.custom1) track.custom1 = metas.custom1;
            if (metas.custom2) track.custom2 = metas.custom2;
            if (metas.custom3) track.custom3 = metas.custom3;
            if (metas.custom4) track.custom4 = metas.custom4;
            if (metas.custom5) track.custom5 = metas.custom5;
            if (metas.custom6) track.custom6 = metas.custom6;
            if (metas.custom7) track.custom7 = metas.custom7;
            if (metas.custom8) track.custom8 = metas.custom8;
            if (metas.custom9) track.custom9 = metas.custom9;
            if (metas.custom10) track.custom10 = metas.custom10;
    ...
    uploadTrack(sync, info, progressCallback) {
        return new Promise<AnyDict>((resolve, reject) => {
    ...
    configPage: {
        summary: {
            ...
            save: function (device, config) {
                mediaSyncDevices.changeLibraryScanSettings(device, {
                    scanToLib: config.qChild('chbScanToLib').controlClass.checked,
                    scan_interval_ms: this._scan_intervals()[0], // continuously
                    downloadToLib: false, // no automatic download
                    syncMetadata: true,
...
window.mediaSyncDevices = {
    ...
    getMetadataStorage(device: Device) {
        let handler = mediaSyncHandlers[device.handlerID];
                        ...
                        if ((!params.handler || params.handler == dev.handlerID) && (params.exceptHandler != dev.handlerID)) {
                            ...
                            if (handler) {
                                ...
                                if (params.scannedToLib || params.scheduledSync) {
                                    ...
                                    if (params.scannedToLib && params.scheduledSync == false)
                                        add = !scheduled && sett.scanToLib;
                                    else
                                    if (params.scannedToLib)
                                        add = sett.scanToLib;
                                    else
                                        add = scheduled;
                                }
    ...
    getCustomSettings(device: Device) {
    ...
    setCustomSettings(device: Device, sett) {
        return new Promise((resolve, reject) => {
            device.customJSON = JSON.stringify(sett);
            device.commitAsync().then(resolve);
        });
    },
    modifyCustomSettings(device: Device, values) {
    ...
    initDevicesArrival() {
        app.listen(app, 'commonChange', (obj, type, device) => {
            if (obj == 'device' && type == 'connected') {
                let thisDevice = device as Device;
    ...
    playlistModified(playlist: Playlist) {
    ...
    changeLibraryScanSettings(device: Device, state) {
        let sett = mediaSyncDevices.getCustomSettings(device);
    ...
    isSyncInProgress(device?: Device /* optional*/) {
    ...
    getSyncTaskInProgress(device: Device, taskType) {
    ...
    checkLibraryScan(device: Device, scanNow: boolean) {
        let sett = this.getCustomSettings(device);
    ...
    initLibraryScan(schedule_timeout: number) {
    ...
    initDownloading() {
        ...
        app.listen(downloader, 'change', (oper, item: DownloadItem) => {
...
export default class MediaSync {
    device: Device;
    private _ERR_TERMINATED: string;
    taskType: string;
    queue: any[];
    ...
    process: any;
    private _fastTrackAccess: any;
    files: StringList;
    isScheduled: boolean;
    private _removingFiles: number;
    private _queuedList: Tracklist;
    ...
    */
    ...
    async sendToDevice(tracks: Tracklist, playlist: Playlist) {
    ...
    */
    ...
    async runAutoSync(runLibraryScan?: boolean) {
    ...
    */
    ...
    removeFromDevice(tracklist?: Tracklist) {
        ...
        let tracks = tracklist.getCopy() as Tracklist;
    ...
    updateDeletionText(count: number) {
        if (count > 1)
    ...
    addPlaylist(playlist: Playlist, tracks: Tracklist) {
        this.taskProgress.text = _('(Preparing list of files)');
    ...
    getStorageInfo() {
        return new Promise<void>((resolve, reject) => {
            ...
            if (syncHandler.getStorageInfo) {
                syncHandler.getStorageInfo(this.device).then(
                    (info) => {
    ...
    removeFiles(filePaths: StringList, tracks: Tracklist) {
        return new Promise<void>((resolve, reject) => {
    ...
    uploadAdditionalFiles(files: StringList) {
            ...
            function copyFile(idx) {
                ...
                if (!_this.taskProgress.terminated) {
                    ...
                    if (syncHandler.uploadFile) {
                        syncHandler.uploadFile(_this, file).then(
                            function () {
                    ...
                    if (syncHandler.downloadFile) {
                        ...
                        let _downloadFile = function(idx) {
                            ...
                            if (!_this.taskProgress.terminated) {
                                ...
                                syncHandler.downloadFile(_this, file).then(
                                    function _resolve() {
    ...
    uploadTracks() {
        ...
        return new Promise<void>((resolve, reject) => {
            ...
            let copyNextTrack = () => {
                ...
                if (this.process.uploadTrackIndex >= files.count)
                    resolve();
                else {
                            ...
                            if (info.needUpload) {
                                ...
                                this.prepareTrackSource(info).then(() => {
                                    if (syncHandler.uploadTrack) {
                                        syncHandler.uploadTrack(this, info, _progressCallback).then(
                                            (res) => {
    ...
    _uploadPlaylists(playlists: SharedList<Playlist>) {
        return new Promise<void>((resolve, reject) => {
            let syncHandler = mediaSyncHandlers[this.device.handlerID];
                    ...
                    if (syncHandler.removeEmptyFolder) {
                        let delFolder = function (idx) {
                            ...
                            if (!_this.taskProgress.terminated) {
                                ...
                                syncHandler.removeEmptyFolder(_this, path).then(
                                    function _resolve() {
    ...
    _finishProcess(success: boolean) {
    ...
    getStreamUrl(device: Device, file) {
        let handler = mediaSyncHandlers[device.handlerID];
    ...
    getStreamUrls(device: Device, files) {
        let handler = mediaSyncHandlers[device.handlerID];
    ...
    _readContentFromMetaFiles(device: Device, info_file?, files?) {
        return new Promise((resolve, reject) => {
            let ms = mediaSyncDevices.getMetadataStorage(device);
            if (!info_file)
    ...
    readMetaFiles(device: Device, sync?:MediaSync /* optional */, retryNumber?:number) {
        let handler = mediaSyncHandlers[device.handlerID];
                ...
                if (sync) {
                    if (sync_lock_file) {
                        ...
                        this.getStreamUrl(device, sync_lock_file).then1((url) => {
                            ...
                            if (lockInfoText == 'OLD_LOCK') { // isn't our lock, but is too old, it is safe to replace it
    ...
    exportMetaFiles(sync: MediaSync) {
                        ...
                        function copyFile(idx) {
                            if ((idx >= files.count) || sync.taskProgress.terminated) {
                                ...
                                _this.removeFile(target).then1(function () {
                                    _this.uploadFile(source, target).then1(
                                        function () {
                                            copyFile(idx + 1);
                                        }
                                    );
    ...
    _acquireSyncLock(metadataStorage: MetadataStorage, sync: MediaSync) {
        let handler = mediaSyncHandlers[sync.device.handlerID];
        return new Promise((resolve, reject) => {
    ...
    _releaseSyncLock(metadataStorage: MetadataStorage, sync: MediaSync) {
        let handler = mediaSyncHandlers[sync.device.handlerID];
    ...
    */
            ...
            if (last_content_signature && syncHandler.getContentChanges) {
                syncHandler.getContentChanges(this.device, last_content_signature, 'scan').then((changes) => {
                    ...
                    asyncForEach(playlists2update, (playlist, next) => {
                        ...
                        this._updateTracks2Lib(changes.tracks.added, false).then(() => { // adds new tracks into local library
                ...
                let _processPlaylist = (idx) => {
                    ...
                    if (!this.taskProgress.terminated) {
                        ...
                        this._updatePlaylist2Library(syncHandler, playlist).then1(() => {
                            this._scanPlaylists(syncHandler, playlist).then(() => { // scan sub-playlists
    ...
    _updateTracks2Lib(tracks: Tracklist, replaceExisting: boolean, isFullScan?: boolean) {
        return new Promise<void>((resolve, reject) => {
