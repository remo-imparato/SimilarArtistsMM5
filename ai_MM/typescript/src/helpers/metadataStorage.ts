...
export default class MetadataStorage {
    handler: MetadataStorageHandler;
    db_dump_items_treshold: number;
    bypass: boolean;
    private _delayedFlushTimeout: number;
    ...
    generateSyncLockFile() : Promise<string> {
        if (this.bypass)
            return dummyPromise('') as Promise<string>;
        else
