...
export default class ArtistTracks extends Control {
    ...
    showingOnline: any;
    private _ds: ArtistDataSource;
    private _dataSourceChangeHandler: (...params: any[]) => void;
    private _onlineDataParsed: boolean;
    onlineEventsSet: boolean;
    myMusicSet: boolean;
    loadingTracklist: boolean;
    private _listChangeHandler: (...params: any[]) => void;
    nextYTPageToken: any;
    ...
    initialize(rootelem, params) {
        ...
        let tlCtrl = UI.artistTracklist.controlClass as ArtistTrackList;
        ...
        if (this._searchPhrase != phrase) { // to not reload unfiltered list via searchBar > restoreLocalSearch > comeOut > searchText = ''  .. to just hide the search bar (#17855)
            ...
            if (!this.showingOnline && !this.showingAllTracks && this._ds) {
                ...
                this._ds.readAllTracks().then((tracks) => {
                    ...
                    (this.UI.artistTracklist.controlClass as ArtistTrackList).autoSortString = this._ds.sortStrings['allTracks']; // prepare right sort string for new data source
                    this.updateView();
                });
            } else {
                this.updateView();
    ...
    setTracklist(getTracklistFunc?, mode?) {
        ...
        if (_this.tracklistMode !== mode) {
            ...
            let tracklistCls = (UI.artistTracklist.controlClass as ArtistTrackList);
    ...
    restorePersistentState(state) {
        ...
        if (this._ds && state.sortStrings) {
            ...
            if (this.showingAllTracks)
                (this.UI.artistTracklist.controlClass as ArtistTrackList).sortString = this._ds.sortStrings.allTracks;
        }
    }

    selectAll() {
        return (this.UI.artistTracklist.controlClass as ArtistTrackList).selectAll();
    }
    ...
    getCurrenSortString() {
        if (this.UI && this.UI.artistTracklist.controlClass)
            return (this.UI.artistTracklist.controlClass as ArtistTrackList).sortString;
        else
            return '';
    }
    ...
    get dataSource() {
        ...
        if (this.tracklistMode && !this.loadingTracklist && this._ds && this.UI && this.UI.artistTracklist.controlClass) {
            this._ds.sortStrings[this.tracklistMode] = (this.UI.artistTracklist.controlClass as ArtistTrackList).autoSortString;
        }
        return this._ds;
    }
    set dataSource(value) {
        ...
        if (this._ds) {
            ...
            if ((this.UI.artistTracklist.controlClass as ArtistTrackList).reportStatus)
...
export class ArtistTrackList extends ColumnTrackList { } // #18456
