...
import ArtworkRectangle from './artworkRectangle';
...
export default class ViewHeader extends Control {
    ...
    private _maxOverflowHeaderPx: number;
    mbDataType: any;
    _promises: AnyDict;
    ...
    initialize(rootelem, params) {
        ...
        this._imageCtrl = UI.itemImageSquare.controlClass as ArtworkRectangle;
    ...
    notfiySizeCouldChanged(el) {
        ...
        if (pEl)
            notifyLayoutChangeDown(pEl as HTMLElement);
    }
        ...
        if (!this.readingWiki) {
            ...
            if (!this.readingWiki) {
                if (data.wikiperex) {
                    ...
                    this.noMatchFilled = false;
                    this.wikidesc = ' ';
                    musicBrainz.fillWikiElement(UI.wikiDescription, '', '&nbsp;', function () { // hard space because of 020807
                        this.notfiySizeCouldChanged(UI.wikiDescription);
                    }.bind(this), {
                        editable: true,
                        fromWiki: false
    ...
    handle_layoutchange(evt?:NotifyEvent) {        
        this.requestFrame(() => {
            this._handle_layoutchange(evt);
        }, '_handle_layoutchange');
    }

    _handle_layoutchange(evt?:NotifyEvent) {
        if (this.dataSource) {
