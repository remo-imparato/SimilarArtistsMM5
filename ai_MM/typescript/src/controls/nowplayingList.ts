...
class NowplayingListView extends ListView {
    standalone: boolean;
    _statusInHeader: boolean;
    _autoScrollToCurrentTrack: boolean;
    headerSet: boolean;
    private _fastSD: Track;
    player: Player;
    _alwaysFollowCurrentTrack: boolean;
    lastPlaybackState: string;
    _lastStatusHTML: string;
    _lastStatusInfo: any;
    private _waitingForLoad: boolean;
    headerStatusBar: HTMLDivElement;
    _lastStatusText: string;
    private _selTimeout: number;
    makeInSync: boolean;
            ...
            this.localListen(this.headerStatusBar, 'click', function () {
                if (window.mainTabs) {
                    ...
                    if (tabIndex < window.mainTabs.length) {
                        ...
                        let tabPanelElement = qid(tabPanel) as ElementWith<MainTabContent>;
