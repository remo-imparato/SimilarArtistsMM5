...
export default class MediaTree extends TreeView {
    onFocusedNodeDeletion: () => void;
    onFocusChange: () => void;
    private _lastHoverDiv: any;
    ...
    contextMenuHandler(e) {
        ...
        this.requestFrame(() => { // #16089 ... wait for next frame to be sure focused item is correctly set
    ...
    deleteSelected(permanent) {
        let node = this.dataSource.focusedNode;
        if (!nodeUtils.isDeleteDisabled(node)) {
            let selList = app.utils.createSharedList() as unknown as SharedList<SharedNode>;
            selList.add(node);
