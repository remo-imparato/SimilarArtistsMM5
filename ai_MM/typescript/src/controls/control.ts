...
export default class Control {
    ...
    _localPromises: Array<Promise<any> | undefined>;
    _dataSourceListenFuncts: (() => void)[];
    disableStateStoring: boolean;    
    mergeParentContextMenu: any;
    closediv?: HTMLDivElement;
    _mouseOutHandler: (evt: NotifyEvent) => void;
    _mouseMoveHandler: (evt: NotifyEvent) => void;
    getCloseButtonAction: () => any;
    setCloseButtonAction: (action: Action) => void;
    ...
    dragging: boolean;
    dragStartHandler: (e: NotifyEvent) => void;
    dragEnterHandler: (e: NotifyEvent) => void;
    dragOverHandler: (e: NotifyEvent) => void;
    dragLeaveHandler: (e: NotifyEvent) => void;
    dropHandler: (e: any) => void;
    dragFinishedHandler: (e?: NotifyEvent) => void;
    activatedHandler: (active: boolean) => void;
    findDNDHandler: (e: NotifyEvent) => any;
    focusEventsRegistered?: boolean;
    touchEventsRegistered?: boolean;
    touchStart: (e: NotifyEvent) => void;
    touchMove: (e: NotifyEvent) => void;
    touchEnd: (e: NotifyEvent) => void;
    _isLongTouch: any;
    _touchDownTime: any;
    _canBeLongTouch: any;
    _cleanFuncs: any;
    _contextMenu: any; // todo Menu
    _dockMenuItems: any;
    ...
    canvas?: any; // todo
    _needsFullRecalculation?: boolean; // splitter
    _layoutChangeHandler?: (evt: NotifyEvent) => void;
    private _helpContext: string;
    _menuArray: Action[];
    ...
    protected __lastFilterPhrase: string;
    _currentLocalFocusIndex: number;
    _localFocusIDsArray: string[];

    constructor(elem: HTMLElement, params?: AnyDict) {
    ...
    initialize(elem: HTMLElement, params?: AnyDict) {
            ...
            if (params.closable && !window.settings.UI.disableRepositioning) {
                ...
                this.setCloseButtonAction = (action) => {
                    ...
                    if (action && action.title) {
                        ...
                        if (title !== _('Close')) title = _('Close') + ' ' + title; // #19448
                    }
        ...
        let _this = this;
        this.statusParams = { // default statusbar params
            visible: function (e) {
                // @ts-ignore
    ...
    set helpContext(value: string) {
        this._helpContext = value;
    }
    get helpContext(): string|undefined {
    ...
    */
    requestFrame(callback: () => void, callbackID?: string) {
        let _this = this;
        if (!this._frameIDs)
            this._frameIDs = {};
    ...
    /**
        ...
        @param callbackID Optional ID for callback, so that multiple instances of the callback do not execute at the same time.
    */
    requestFrameMM(callback: () => void, callbackID?: string) {
        let _this = this;
        if (!this._frameMMIDs)
            this._frameMMIDs = {};
    ...
    */
    requestTimeout(callback: () => void, timeMS: number, callbackID?: string, useFirst?: boolean) {
        if (!this._timeoutIDs)
            this._timeoutIDs = {};
        if (callbackID) {
    ...
    */
    requestIdle(callback, callbackID?: string, useFirst?: boolean) {
    ...
    enableDragNDrop() {
        ...
        let addDragMethods = (e: NotifyEvent) => {
            e._target = e.target;
        ...
        let isDragAllowed = function (e: NotifyEvent) {
        ...
        this.dragStartHandler = (e: NotifyEvent) => { };

        this.dragEnterHandler = (e: NotifyEvent) => {
            if (isDragAllowed(e)) {
                addDragMethods(e);
                this.dragEnter(e);
            }
        ...
        this.dragOverHandler = (e: NotifyEvent) => {
            if (isDragAllowed(e)) {
                e.preventDefault();
                addDragMethods(e);
                if (this.canDrop(e)) {
        ...
        this.dragLeaveHandler = (e: NotifyEvent) => {
            if (isDragAllowed(e)) {
                addDragMethods(e);
                this.dragLeave(e);
                this.dragging = false;
        ...
        this.dropHandler = (e: NotifyEvent) => {
            if (isDragAllowed(e)) {
                e.preventDefault();
                e.stopPropagation();
                addDragMethods(e);
        ...
        this.dragFinishedHandler = (e?: NotifyEvent) => {
            this.dragFinished(e);
        };

        this.activatedHandler = (active: boolean) => {
        ...
        this.findDNDHandler = function (e: NotifyEvent) {
            // browse parents
    ...
    focusHandler(element: HTMLElement, newState) {
    ...
    */
    ...
    @param {object} Event object. dataTransfer property is used for D&D operations
    */
    drop(e: NotifyEvent, isSameControl?: boolean) {
        let handler = this.findDNDHandler(e);
        if (handler && (handler != this.container)) {
    ...
    */
    ...
    fileTransferPrepare(element, e: NotifyEvent) {
    ...
    */
    makeDraggable(element, canBeDragged?: boolean) {
    ...
    */
    raiseEvent(eventName, details?: AnyDict, isCancelable?: boolean, canBubble?: boolean, sender?: EventTarget) {
        type EventParams = {
            detail: AnyDict;
            cancelable?: boolean;
            bubbles?: boolean;
        ...
        let senderObject: EventTarget = this.container;
        if (sender)
            senderObject = sender;
    ...
    */
    addControl(controlClass: string, initParams?: AnyDict) {
    ...
    */
    ...
    restoreState(fromObject: ControlState, isJustViewModeChange?:boolean) {
    ...
    getPersistentStateRootKeyBase() {
        ...
        if (this.parentView) {
            ...
            if (!key && coll) {
                ...
                if (sett.mode == 'perCollection')
                    key = nodeUtils.getCollectionStateRootKey(coll);
                else
                if (sett.mode == 'perCollections')
    ...
    */
    ...
    registerStatusBarSource(source) {
        if (source) {
            ...
            this.requestFrame(function () { // LS: moved to RAF as this can take more than 16 ms 
    ...
    */
    ...
    openView(nodeDataSource, nodeHandlerID, clickedArea, newTab?:boolean) {
        let view = this.parentView;
        if (view) {
            view.openSubNode({
    ...
    */
    ...
    dataSourceListen(object, event, func, capture?: boolean) {
    ...
    */
    thenWithProgress(pPromise, params) {
        ...
        let taskProgress: SimpleTasksController | BackgroundTask | undefined = undefined;
    ...
    contextMenuHandler(e) {
        ...
        if (this._contextMenu) {
            ...
            // @ts-ignore
            const willShowInPanel: boolean = (window.mainMenuPanel && (this instanceof ListView) && uitools.startSelectionMode(this));
            ...
            if (willShowInPanel) {
                ...
                if (menu && menu.menuHandler && menu.menuHandler.menuitems && (!isArray(menu.menuHandler.menuitems) || (menu.menuHandler.menuitems.length > 0))) {
                    ...
                    if (byKey || (screenX === undefined)) { // PETR: don't know why, but sometimes when using touches screenX/Y is undefined!
                        ...
                        let focusedEl: HTMLElement | undefined = undefined;
    ...
    */
    ...
    registerEventHandler(event, capture?: boolean) {
    ...
    unregisterEventHandler(event: string, capture?: boolean) {
        let obj = this.container as any; // "as any" is temporary
        if (!capture) {
    ...
    @param capture
    */
    localListen(object: any, event: string, func: AnyCallback, capture?: boolean) {
        let listenFunc = app.listen(object, event, func, capture);
    ...
    */
    ...
    set hasSplitters(value: boolean) {
        if (!this.container)
            return;
    ...
    setFocusedAndSelectedIndex(idx: number): Promise<unknown> {
        // overriden in descendants
        return dummyPromise();
    }

    setItemFullyVisibleCentered(idx: number) {
        // overriden in descendants
    }

    setItemFullyVisible(idx: number) {
        // overriden in descendants
    }
    ...
    public get menuArray(): any[] {
        return this._menuArray;
    }
    public set menuArray(value: any[]) {
...
/** @internal */
export type MethodsDict = Dictionary<(...params: any) => any>;
/** @internal */
export type GetterSetterDict = Dictionary<{ get?: () => any, set?: (value: any) => void }>;


export type ControlState = {
    hasSplitters?: boolean;
    manualVisibilityState?: boolean;
    [key: string]: any; // JL: added to allow other controls to store their own stuff
}
/**
 * Format for {@link Control.storeState} to store its state in persistent.json.
 */
export type StoreStateResult = ControlState;
...
*/
export function inheritClass(className: string, parent: any, methods: MethodsDict, properties?: GetterSetterDict) {
    window[className] = inherit(className, parent, methods, properties);
}
window['inheritClass'] = inheritClass;

export function classCreation(this: Control, rootelem: HTMLElement, params: AnyDict) {
    if (this.initialize)
...
export function inherit(className: string, parent: any, methods: MethodsDict, properties?: GetterSetterDict) {
    ...
    let newclass = new Function(
        'return function ' + className + '() {"use strict";classCreation.apply(this, arguments)}'
    )();
...
export function inherited(method: string, control: Control, args: any[]) {
...
export function getPixelSize(val, attr?, ctrl?): number {
    if (!isNaN(Number(val))) {
        return val;
    } else {
...
export function getMinWidth(div: HTMLElement, cs?: CSSStyleDeclaration): number {
    let retval = 0;
    if (div.controlClass) {
...
export function getMinHeight(div: HTMLElement, cs?: CSSStyleDeclaration): number {
    let retval = 0;
    if (div.controlClass) {
...
declare global {
    interface DragEvent {
        _target?: EventTarget;
        _dropTargetControl?: HTMLElement;
    }
    interface DataTransfer {
        getSourceControl?: () => HTMLElement|null;
        isSameControl?: () => boolean;
        srcObject?: () => any|null;
        setDropTargetControl?: (ctrl: HTMLElement) => void;
    }
}