...
class ListEditButtons extends Control {
    lastItemEditable: boolean;
    firstItemDeletable: boolean;
    buttons: {[key: string]: HTMLElement;};
    deletedItems: any[];
    _list: DataSource<any>;
    listView: ElementWith<ListView>;
    private _beforeDeleteMethod: () => Promise<void>;
    private __lastChangeListener: (...params: any[]) => void;
    itemEditableCallback: (item: any) => boolean;
    itemDeletableCallback: (item: any) => boolean;
    itemMovableableCallback: (item: any, focused: number, isUp: boolean, list: DataSource<any>) => boolean;
