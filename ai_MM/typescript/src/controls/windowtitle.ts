...
class WindowTitle extends Control {
    ...
    resetMoveable: () => void;
    private _border_left: HTMLDivElement;
    private _border_right: HTMLDivElement;
    private _border_top: HTMLDivElement;
    private _border_bottom: HTMLDivElement;
    private _icon: string;
    private _borderSize: number;
    private _disableWindowMove: boolean;
    ...
    initialize(elem, params) {
        ...
        let statChange = function (maximized, minimized) {
            if (isChildOf(document.body, _this.container)) {
                ...
                if (min) setVisibility(min, resizeable && !minimized && canMinimize);
                if (res) setVisibility(res, resizeable && ((maximized && canMaximize) || (minimized && canMinimize)));
                if (max) setVisibility(max, resizeable && !maximized && canMaximize);
        ...
        this.localListen(thisWindow, 'visibilitychanged', function (min, vis, state) { // state: 0 normal, 1 minimized, 2 maximized
    ...
    _handleBorders() {
        if (!window.webApp) {
            ...
            if (this._border_left) setVisibility(this._border_left, shouldBeVisible);
            if (this._border_right) setVisibility(this._border_right, shouldBeVisible);
            if (this._border_top) setVisibility(this._border_top, shouldBeVisible);
            if (this._border_bottom) setVisibility(this._border_bottom, shouldBeVisible);
