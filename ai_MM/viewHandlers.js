...
'use strict';
import { inherit } from './controls/control';
...
export const viewHandlers = {
        ...
        onShow: function (control, view) {
            ...
            let __refreshTracklist = function (eventType, subType) {
                if (eventType == 'tracklist') {
                    ...
                    else if (_thisHandler.getDataSource) {
                        list = _thisHandler.getDataSource(view);
                    }
                    else if (view.viewNode.dataSource) {
    ...
    columnBrowser: inheritHandler('ColumnBrowser', 'Base', {
        ...
        onShow: function (control, view, mainViewControl) {
            ...
            let _changeList = function (list, assignAsOrig, restoreSelection) {
            ...
            view.listen(mainViewControl, 'datasourcefiltered', function (e) {
                ...
                else if (e.detail.original_tracklist) {
    ...
    playlist_browser: inheritHandler('Playlist_browser', 'Playlist_tracklist', {
        ...
        columnsSupport: false,
        icon: 'browser',
        storeStateKey: 'playlistsBrowser',
        baseViewType: 'list',
    ...
    playlistsListView: inheritHandler('PlaylistsListView', 'NodeList', {
        ...
        noAnimations: true,
        baseViewType: 'list',
        storeStateKey: 'playlistsSharedBase',
    ...
    imageGrid: inheritHandler('ImageGrid', 'BaseList', {
        ...
        onShow: function (control, view) {
            ...
            else if (handler.getViewDataSource) {
            ...
            let _refreshSource = () => {
                ...
                if (!albumList) {
                    ...
                    if (!tracks) {
                        ...
                        else if (view.viewNode.dataSource && view.viewNode.dataSource.getTracklist)
    ...
    playlist_albumlist: inheritHandler('Playlist_AlbumList', 'AlbumList', {
        ...
        storeStateKey: 'playlistsAlbumList',
        hiddenSubViews: ['columnBrowser', 'playlist_RowNodeList'],
        subViews: ['columnBrowser', 'playlist_header', 'playlist_Tree', 'playlist_RowNodeList', 'playlist_NodeList', 'statusBar']
    }),
    ...
    newEpisodes: inheritHandler('NewEpisodes', 'ImageGrid', {
        ...
        onShow: function (control, view) {
            control.controlClass.dataSource = app.podcasts.getPodcastListBySQL('SELECT Podcasts.* FROM Podcasts WHERE ID IN (SELECT IDPodcast FROM PodcastEpisodes WHERE Downloaded = 1) ' +
    ...
    webBrowser: inheritHandler('WebBrowser', 'Base', {
        ...
        onShow: function (control, view) {
            ...
            if (view.viewNode.dataSource) {
                ...
                else if (view.viewNode.dataSource.title) {
    ...
    albumview_header: inheritHandler('Albumview_header', 'Base', {
        ...
        onShow: function (control, view, mainViewControl) {
            ...
            control.controlClass.requestFrame(function () {
...
export const nodeUtils = {
    getNodeTitle: function (node, incChildCount, acceptPromises, forEditing) {
    ...
    getNodeHelpContext: function (node) {
        let handler = nodeHandlers[node.handlerID];
    ...
    getMultiselectSupport: function (node) {
        let handler = nodeHandlers[node.handlerID];
        if (!handler)
            return false;
        return resolveToValue(handler.multiselect, false);
    },
    getHasVirtualChildrenSupport: function (node) {
        let handler = nodeHandlers[node.handlerID];
        if (!handler)
            return false;
    ...
    getHasChildren: function (node) {
        ...
        if (isFunction(handler.hasChildren))
            ...
            return handler.hasChildren;
    },
    getHasTreeChildren: function (node) {
        let handler = nodeHandlers[node.handlerID];
        if (!handler)
            return false;
        if (handler.hasTreeChildren != undefined) {
            if (isFunction(handler.hasTreeChildren))
                ...
                return handler.hasTreeChildren;
        }
        else {
    ...
    initLoadProgress: function (node) {
    ...
    closeLoadProgress: function (node) {
        if (nodeUtils.progress) {
            nodeUtils.progress.terminate();
            nodeUtils.progress = null;
        }
    },
    refreshNodeChildren: function (node, withoutProgress) {
        let handler = nodeHandlers[node.handlerID];
    ...
    deferredRefresh: function (node, useTimeout) {
        node.tag = node.tag || {};
    ...
    deferredRefreshCancel: function (node) {
        clearTimeout(node.tag.__refreshTimeout);
    },
    ...
    fillFromList: function (node, list, handlerID) {
        return new Promise(function (resolve, reject) {
            if (list.isLoaded) {
    ...
    storeDataSource: function (dataSource) {
        if (!dataSource)
            return null;
    ...
    restoreDataSource: function (dataSource) {
        return new Promise(function (resolve, reject) {
            if (dataSource) {
                if (dataSource.objectType) {
                    if (dataSource.objectType == 'QueryData')
                        ...
                        if (dataSource.persistentInfo) {
                            app.getObject(dataSource.objectType, dataSource.persistentInfo).then(function (DS) {
                                resolve(DS);
                            }, function (err) {
    ...
    restoreNode: function (parentNode, state, addToTree) {
        let _this = this;
        return new Promise(function (resolve, reject) {
            _this.restoreDataSource(state.dataSource).then(function _res(dataSource) {
    ...
    getNodeByPath: function (rootNode, pathNodeStates) {
    ...
    getNodeCollection: function (node) {
        if (node.collection)
            return node.collection; // does the same as the commented code below, but is faster
        let n = node;
        let idColl = 0;
    ...
    getParentAtLevel: function (node, level) {
        let n = node;
        while (n && n.level >= 0) {
            if (n.level == level)
                return n;
    ...
    getNodeStateRootKey: function (node, coll) {
        let n = node;
        while (n && n.level >= 0) {
    ...
    getCollectionStateRootKey: function (coll) {
        return 'CONTROLS_STATE_COLLECTION_' + coll.id;
    },
    getCanBeCollapsed: function (node) {
        let handler = nodeHandlers[node.handlerID];
        if (!handler)
            return false;
        return resolveToValue(handler.canBeCollapsed, true);
    },
    _collapseNode: function (node, forced) {
        let _this = this;
    ...
    collapseNode: function (rootNode, forced) {
        let _this = this;
    ...
    collapseNodeList: function (list, forced) {
        let _this = this;
    ...
    _getNodeHandlersStoreKey: function (node) {
    ...
    setNodeHandlerState: function (viewData, state) {
    ...
    getNodeHandlerState: function (viewData) {
        let node = viewData.viewNode;
        let key = this._getNodeHandlersStoreKey(node);
        let res = undefined;
        let states = app.getValue(key, {});
        ...
        if (!inArray(handlerState.viewAsId, _viewAs) && handlerState.baseViewType) {
            ...
            if (!window.views.getCompatibleView(viewData, handlerState.viewAsId)) {
                ...
                forEach(_viewAsArr, (key) => {
                    ...
                    else if (viewHandlers[key].baseViewType && (viewHandlers[key].baseViewType == handlerState.baseViewType))
    ...
    _getViewHandlersStoreKey: function (node) {
    ...
    setViewHandlerState: function (viewData, state) {
        let node = viewData.viewNode;
    ...
    getViewHandlerState: function (viewData) {
        let node = viewData.viewNode;
        let key = this._getViewHandlersStoreKey(node);
        let res = undefined;
        let states = app.getValue(key, {});
    ...
    isDeleteDisabled: function (node) {
    ...
    copyNodeItem: function (node, cut) {
        if (node.dataSource) {
            window.clipboard = {
    ...
    getBrowserTracksSortString: function () {
    ...
    getNodesTracklist: function (nodes) {
    ...
    createContextMenu: function (getNodes) {
        ...
        function createTracklistEditAction(action, allowReadOnly) {
            return {
                title: action.title,
                icon: action.icon,
                visible: function () {
                    if (!window.uitools.getCanEdit() || !isTracklistNode())
                        return false;
                    else if (!allowReadOnly && node && node.handlerID) {
        ...
        if (this._addDefaultMenu(node)) {
            ...
            result.push({
                action: {
                    ...
                    execute: function () {
                        let handler = nodeHandlers[node.handlerID];
                        window.uitools.pasteClipboard(function _canPaste(cdata) {
    ...
    getDevicesBy: function (node, params, handler) {
        return new Promise(function (resolve, reject) {
...
function inheritNodeHandler(className, parentClass, methods, properties) {
    nodeHandlersClasses[className] = inherit(className, nodeHandlersClasses[parentClass], methods, properties);
    return new nodeHandlersClasses[className]();
}
// @ts-ignore
...
nodeHandlers.playlist = inheritNodeHandler('Playlist', 'PlaylistBase', {
    ...
    viewAs: ['playlist_browser', 'playlist_tracklist', 'playlist_groupedTracklist', 'playlist_albumlist' /*, 'playlist_trackgrid'*/],
    multiselect: true,
...
nodeHandlers.subscriptions = inheritNodeHandler('Subscriptions', 'Observable', {
    ...
    icon: 'podcast',
    getViewDataSource: function (view, dataSourceType) {
        ...
        messageDlg(message, 'Confirmation', ['btnYes', 'btnNo'], {
            ...
            if (result.btnID === 'btnYes') {
                ...
                nodes.forEach(function (node) {
                    ...
                    else if (node.dataSource.getPathAsync) { // e.g. db folders
...
nodeHandlers.track = inheritNodeHandler('Track', 'Base', {
    title: function (node) {
        if (node.dataSource) {
            ...
            if (id === 'radio')
                return node.dataSource.album || node.dataSource.title;
        }
        return node.dataSource.title;
    },
    icon: function (node) {
...
nodeHandlers.cloudFolder = inheritNodeHandler('CloudFolder', 'BaseFolder', {
    ...
    getChildren: function (node) {
        return new Promise(function (resolve, reject) {
            ...
            if (!service)
                ...
                service.listContent(node.dataSource.path).then(function (content) {
    ...
    getViewDataSource: function (view) {
        ...
        if (service && service.isAuthorized()) {
            service.listContent(dataSource.path, list).then(function (content) {
                if (list.count == 0)
...
nodeHandlers.devicesList = inheritNodeHandler('DevicesList', 'Base', {
    ...
    helpContext: 'My Computer',
    getChildren: function (node) {
        node.loadPromise = new Promise((resolve) => {
    ...
    getViewDataSource: function (view) {
        ...
        if (uitools.globalSettings.showingOnline) {
            ...
            if (view.dataSourceCache['onlineTracklist']) {
                let tl = view.dataSourceCache['onlineTracklist'];
                ...
                let tl = app.utils.createTracklist();
            ...
            let tl = view.viewNode.dataSource.getTracklist();
            view.promise(tl.whenLoaded()).then(function () {
                list.addList(tl);
                list.notifyLoaded();
                list.globalModifyWatch = true;
...
nodeHandlers.genreCategory = inheritNodeHandler('GenreCategory', 'AutoUpdateParent', {
    ...
    getViewDataSource: function (view, dataSourceType) {
        ...
        else if (dataSourceType == 'genrelist')
            return view.viewNode.dataSource.getGenreList();
        else if (dataSourceType == 'albumlist')
...
nodeHandlers.tempo = inheritNodeHandler('Tempo', 'ClassificationParent', {
    ...
    getChildren: function (node) {
        let col = nodeUtils.getNodeCollection(node);
        return nodeUtils.fillFromList(node, app.db.getTempoList(col.id), 'classificationnode');
    },
    viewAs: function (node) {
        let col = nodeUtils.getNodeCollection(node);
        let ctype = undefined;
...
nodeHandlers.mood = inheritNodeHandler('Mood', 'ClassificationParent', {
    ...
    getChildren: function (node) {
        let col = nodeUtils.getNodeCollection(node);
        return nodeUtils.fillFromList(node, app.db.getMoodList(col.id), 'classificationnode');
    },
    viewAs: function (node) {
        let col = nodeUtils.getNodeCollection(node);
        let ctype = undefined;
...
nodeHandlers.occasion = inheritNodeHandler('Occasion', 'ClassificationParent', {
    ...
    getChildren: function (node) {
        let col = nodeUtils.getNodeCollection(node);
        return nodeUtils.fillFromList(node, app.db.getOccasionList(col.id), 'classificationnode');
    },
    viewAs: function (node) {
        let col = nodeUtils.getNodeCollection(node);
        let ctype = undefined;
...
nodeHandlers.quality = inheritNodeHandler('Quality', 'ClassificationParent', {
    ...
    getChildren: function (node) {
        let col = nodeUtils.getNodeCollection(node);
        return nodeUtils.fillFromList(node, app.db.getQualityList(col.id), 'classificationnode');
    },
    viewAs: function (node) {
        let col = nodeUtils.getNodeCollection(node);
        let ctype = undefined;
...
nodeHandlers.web = inheritNodeHandler('Web', 'Base', {
    ...
    onExpanded: function (node) {
        ...
        app.listen(this._list, 'change', node.tag.onWebsChanged);
    },
    onCollapsed: function (node) {
        nodeUtils.deferredRefreshCancel(node);
        if (this._list) {
...
nodeHandlers.radio = inheritNodeHandler('radio', 'Observable', {
    ...
    onExpanded: function (node) {
        ...
        app.listen(this._list, 'change', node.tag.onRadioChanged);
    },
    onCollapsed: function (node) {
        nodeUtils.deferredRefreshCancel(node);
        if (this._list) {
...
if (!window.isMainWindow) { // #19269
    ...
    navUtils = {
        createNodeState: function (handlerID, dataSource) {
            let tempTree = app.createTree();
            let tempNode = tempTree.root.addChild(dataSource, handlerID);
            return nodeUtils.storeNode(tempNode);
        },
        navigateNodePath: function (nodePathSources, resolveWhenFinished /* set true, if you want to be notified about when view switch finished */) {
            return new Promise(function (resolve, reject) {
                ...
                nodeUtils.getNodeByPath(mainTabContent.rootNode, nodePathSources).then(function (newNode) {
                    mainTabContent.showView({
                        node: newNode,
        ...
        navigateObject: function (params) {
        ...
        getTrackCollectionAsync: function (track) {
        ...
        getOutOfActiveView: function (nav2Node) {
        ...
        getFocusedNode: function () {
            if (lastFocusedControl && lastFocusedControl.controlClass) {
                ...
                else if (lastFocusedControl.controlClass.constructor.name == 'MediaTree') {
        ...
        filePath2nodePath: function (filePath) {
            let res = [];
                ...
                if (object.objectType == 'track') {
                    ...
                    let _navTrack = function (collection) {
                        app.getObject('album', {
                            ...
                            track.getTrackTypeAsync('stringId').then(function (tt) {
    ...
    artist: {
        navigate: function (object, collection, mbgid, personID) {
            return new Promise(function (resolve, reject) {
                ...
                else if ((object.objectType === personID) || ((personID === 'albumartist') && (object.objectType === 'artist'))) {
                    // test, if this is really Delphi class or just helper JS class
                    if (object.jsclass) {
                ...
                else if ((object.objectType == 'album') && ((personID == 'artist') || (personID == 'albumartist'))) {
            ...
            return new Promise(function (resolve, reject) {
                app.collections.getCollectionForObjectAsync(object).then(function (collection) {
                    navUtils.navigateObject({
                        ...
                        parentNodeHandlerID: object.objectType + 's',
                        object: object,
                        collection: collection
                    }).then(resolve, reject);
                });
